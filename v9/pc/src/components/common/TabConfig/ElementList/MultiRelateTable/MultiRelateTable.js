...
import { getElementMultiRelation } from '@/services/pageConfig';
...
class MultiRelateTable extends Component {
  ...
  fetchData = () => {
    ...
    getElementMultiRelation({ elementId }).then(multiRelateList => {
      this.setState({
        loading: false,
      });
      this.props.setWrapState({ multiRelateList });
  ...
  handleAddRow = () => {
    ...
    this.props.setWrapState({ multiRelateList: newList });
  }
  // 删除数据
