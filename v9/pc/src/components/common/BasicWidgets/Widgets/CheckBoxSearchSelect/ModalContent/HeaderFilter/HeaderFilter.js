 ...
 * @Last Modified time: 2022-05-27 11:47:09
 * @Desc: 头部修改显示状态容器，包括：
 *  1. 切换树、列表
 *  2. 显示格式（名称、类型、状态）
 */
...
class HeaderFilter extends Component {
  ...
  render() {
    const { showType, setWrapState, eleObj, sourceSelectedRows, treeNodeType } = this.props;
    ...
    return (
      <div className={`${styles.wrap} system__lineBorder--bottom`}>
        {(selectShowType === 1 || selectShowType === 3 || selectShowType === null) && (
          <span
            className={treeIcon}
            onClick={() => setWrapState({ showType: 'tree', selectedRows: sourceSelectedRows })}
          />
        )}
        {(selectShowType === 2 || selectShowType === 3 || selectShowType === null) && (
          <span
            className={listIcon}
            onClick={() => setWrapState({ showType: 'list', selectedRows: sourceSelectedRows })}
          />
        )}
