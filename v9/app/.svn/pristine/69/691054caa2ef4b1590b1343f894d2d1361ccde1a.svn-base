<template>
  <view class="inspection-device">
    ...
    <view
      ...
      v-else-if="inspectionDeviceLock === '1' || inspectionDeviceLock === '3'"
    >
      ...
      <!-- #endif -->
      <!-- #ifndef APP-PLUS -->
      <view class="item scan" v-if="isShowScan && QRType !== '0'" @click="handleScan">
      <!-- #endif -->
        <m-font-icon
          class="item-icon"
          icon="iconsaoma1"
      ...
      <view
        class="item unlock"
        v-if="enableInspectionUnlockByCode === 'true'"
        @click="handleUnlock"
      >
        <m-font-icon
          class="item-icon"
...
export default {
  ...
  computed: {
    ...
    ...mapState('Inspection', [
      'inspectionDeviceLock',
      'enableInspectionUnlockByCode',
      'limitSkipToToday',
      'constraintDeviceCompleteInspection',
    ]),
    // 是否启用rfid
    isShowRfidHR() {
  ...
  onBackPress(options) {
    if (
      !this.isAutoHours &&
      this.hourFillingAutomatic === '2' &&
      this.constraintDeviceCompleteInspection == '2'
    ) {
      updateSql('inspectiondo', { manhours: 0 }, { id: this.id });
    }
  },
  methods: {
    ...
    handleJumpClick(device) {
      ...
      if (this.isSelectDevie) {
        ...
        this.inspectionDeviceLock === '2' ||
        this.inspectionDeviceLock === '3'
      ) {
    ...
    handleFinish() {
      const that = this;
      let iscomplete = true;
      if (this.constraintDeviceCompleteInspection == '1') {
        const arr = [];
        this.deviceData.map((item) => {
          if (
            item.moOperatingCount != 0 &&
            item.moOperatingCount < item.moPointCount
          ) {
            arr.push(item);
          }
          return item;
        ...
        if (arr.length > 0) {
          ...
          arr.map((item, index) => {
            ...
            console.log(item.moname);
          });
          uni.showModal({
            title: '提示',
            content: `${str}等设备未全部点检完成`, // 中文：'有强制点没有完成',
            showCancel: false,
            confirmText: that.$t('basic-confirm'),
          });
        }
