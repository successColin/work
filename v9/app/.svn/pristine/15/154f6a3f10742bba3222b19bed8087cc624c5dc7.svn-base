...
const getWechatSSOLogin = 'sso/wechatSSOLogin'; //浙政钉授权
/**
 * 阿斯利康单点登录
...
/**
 * 浙政钉
 * 地址中注入用户信息code:用户账号，根据code获取系统用户信息
 */
 ...
 * 地址中注入用户信息code:用户账号，根据code获取系统用户信息
 */
export const APP_EnterpriseWechatLogin = async (param, _this) => {
  let userInfo = {};
  ...
  if (code) {
    ...
    try {
      await _this.$http
        .post(getWechatSSOLogin, param) //通过获取到的code获取本系统的用户信息
        .then((res) => {
          const result = res.data || {};
          const { success, data } = result;
          userInfo = data;
