			...
			<view class="uni-calender__body-date-week calendar-items-week" :class="[themeClass_]">
				<block v-for="(day, index) in weeks" :key="index">
					<view class="item" :class="{
						...
						<view class="hasData" v-if="isHasData(day)" style="bottom: 0px;width: 6px;height: 6px;margin-left: -2.5px;z-index: 0" />
						<!-- <view :class="{ 'uni-calender_check': day.checked, 'calender_check-begin': day.multipleBegin, 'calender_check-end': day.multipleEnd }" class="uni-calender_check-bg uni-calender_check" /> -->
						<!-- <view class="circle-box" v-if="((day.date == canlender.date && !day.checked) || day.multipleBegin || day.multipleEnd) && canlender.month == day.month && !day.disable"></view> -->
					</view>
				</block>
			</view>
		</block>
