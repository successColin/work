  ...
  <view class="calendar-page" :class="[themeClass_]">
    ...
    <view class="calendar-page-content">
      ...
      <view class="calendar-page-data">
        <scroll-view
          scroll-y
          class="calendar-page-data-scroll"
        >
          <block v-for="(item, index) in calendarContent" :key="index">
            <view
              ...
              "
							v-show="showListData[item.mobileTabId]"
            >
              <view class="title">{{
...
export default {
  ...
  methods: {
    ...
    ...mapMutations('common', ['SET_TEMPLATE_PARAMS', 'SET_TABS_ELEMENT']), //设置面板变量
		// 需要实现如果列表中没有数据，对应的模块也不显示
		canShowItem(stateObj){
			this.showListData = {...this.showListData,...stateObj}
		},
    clickList(params) {
      const _this = this;
    ...
    setPageInfo(mobileTabs, bundleId) {
      ...
      this.isShows[relationTab] = true;
			this.showListData[relationTab] = false
