    ...
    <view class="myself-content">
      <view class="user-bundle-list">
        ...
        <view
          ...
          v-on:click="
            openUrl(
              'https://v10.mingcloud.top/onlineApp/hybrid/html/web/viewer.html?file=https://v10.mingcloud.top:9000/app/%E9%9A%90%E7%A7%81%E5%8D%8F%E8%AE%AEv9.pdf'
            )
          "
        >
          <view class="font-icon">
          ...
          <view class="bundle-name">{{ $t('page-privacy') }}</view>
          <view class="arrow-right">
            <!-- <span></span> -->
            <m-font-icon
              icon="iconyuefenqiehuanhou"
        ...
        success: function (res) {
          if (res.confirm) {
            for (let i = 0; i < _this.storageKeys.length; i++) {
              console.log(_this.storageKeys);
              let key = _this.storageKeys[i];
              // 清除缓存后配置按钮消失了，下次要清除的具体写
              // if (
              //   key != 'language' &&
              //   key != 'baseUrl' &&
              //   key != 'inTestServer' &&
