...
let listTem = new ClassList(),
  mobileTabs = '';
export default {
  components: { topBtnsComs, mInputSearch, ListCom, bottomBtnsComs },
  data() {
    return {
      //列表属性
      listConfig: {},
  ...
  methods: {
    ...
    setPageInfo(mobileTab, mainTabID) {
      ...
      listConfig.baseParams = {
        ...
        multiple: true,
        hasCheck: enableSelectBox === 1 ? true : false, // 列表是否需要多选
      };
    ...
    if (isNew || JSON.stringify(mobileTabs) === '{}') {
      ...
      homeApi
        ...
        .then((value) => {
          ...
          console.log(22222222, mobileTabs, mainTabID);
      ...
      console.log(33333, mobileTabs.mobileTabs, mobileTabId);
      this.setPageInfo(mobileTabs.mobileTabs, mobileTabId);
    }
    // 是否全列表是否启动多选功能
    // this.$http
    //   .post('bundle/isEnabledById', {
    // 		id: tabInfo.bundleId,
    // 	})
    //   .then((res) => {
    //     console.log(res)
    // 		if (res.data.success) {
    // 			this.isEnabled = res.data.data;
    // 		}
    //   }).then(() => {
    // 	});
    //设置监听
