const mutations = {
  ...
  SHOW_UPLOAD(state, show) {
    //是否显示更新框
    state.showUpload = show;
  },
  SET_FRAMETYPE(state, frameType) {
    //设置框架类型
    state.frameType = frameType;
  },
  SET_VERSIONINFO(state, verisioninfo) {
    //设置版本信息
  ...
  SET_SYSPARAMER(state, param) {
    //设置全局变量
    state[param.key] = param.value;
  },
  SET_WATERMARK(state, param) {
    //设置水印全局变量
    state.waterMarkConfig = param;
  },
  SET_QRTYPE(state, param) {
    // 设置H5扫一扫变量
    state.QRType = param;
  },
  ...
  SET_FORM_PickerShow(state, flag) {
    //是否有下拉框张开
  ...
  UPDATE_USER_STATE(state, params) {
    //更新个人信息
    state.userImag = params.picpath;
    state.email = params.email;
    state.telephone = params.telephone;
  ...
  UPDATE_SERVERURL_STATE(state, params) {
    //更新服务器url
    state.baseUrl = params;
  },
  UPDATE_THEME_COLOR(state, params) {
    //更新服务器url
    state.themeColor = params.color;
    state.themeColorCode = params.colorCode;
  },
  SET_LOGIN_CONFIG(state, params) {
    //设置版本信息
    state.appLoginLogo = params.appLoginLogo;
    state.technicalSupport = params.technicalSupport;
  },
  SET_SCANTYPE(state, param) {
    //设置扫描是否海立
    state.scanningType = param;
  },
  SET_HOURFILLINGAUTOMATIC(state, param) {
    state.hourFillingAutomatic = param;
  },
  UPDATE_MESSAGE_NUM(state, params) {
    //更新消息推送的条数
