...
const selectMoList = function(inspectionDoId, cb, cbe) {
	let sql =
		`SELECT
			...
			WHERE motable.inspectiondoid = ${inspectionDoId} AND motable.mokey = inspectionpointdo.mokey) AS abnormalMoCount,
			(SELECT COUNT(*)
			FROM (SELECT mokey,inspectiondoid 
					FROM inspectionpointdo 
			...
			WHERE motable.inspectiondoid = ${inspectionDoId} AND motable.mokey = inspectionpointdo.mokey) AS shutdownMoCount,
...
const dropInspectionTables = function(cb, cbe) {
	// 在iOS上，如果操作过db，但没有关闭db，如果再进行热更新，会导致资源未释放
	// 从而无法删除表，导致无法重新下载
	DB.closeDB(function() {
...
const updateManhourSql = function(tableName, ary, cb, cbe) {

	let idStr = ary.join(',');
	let sql = `UPDATE ${tableName} SET manhours = null WHERE id in (${idStr});`
