        ...
        >
          <view class="detail-form">
            ...
            <view class="basic-info" v-if="item.checktype != 999">
              ...
              <view class="basic-info-item" v-if="item.checktype > 3 && item.checktype != 9">
                <text class="label-text"
              ...
              <view
                ...
                v-if="(item.checktype > 3 || item.checktype == 2) && item.checktype != 9"
              >
                <text class="label-text"
            ...
            <view
              ...
              v-else-if="(item.checktype > 3 && item.checktype != 999) && item.checktype != 9"
            >
              <view class="info-item">
                <text class="label-text"
                  >{{ $t('inspection-measuredUnit') }}：</text
            ...
            <block v-if="item.checktype != 999">
              <view
                class="operation"
                v-if="item.checktype == 1 || item.checktype == 3 || item.checktype == 9"
              >
                <text
...
export default {
  ...
  onBackPress() {
	...
	if (this.constraintDeviceCompleteInspection == '1') {
		const arr = this.pointList.filter((item) => item.stateid == 0)
		if (arr.length > 0 && arr.length < this.pointList.length){
			backFlag = true;
		  uni.showModal({
			title: this.$t('basic-modalTip'), //中文：'提示',
			content: '点检任务未完成', // 中文：'异常状态下执行描述不能为空!',
			// content:this.$t('inspection-enterMeasuredValue'),// 中文：'请填写测量值',
			showCancel: false,
			confirmText: this.$t('basic-confirm'),
		  });
  ...
  computed: {
    ...
    ...mapState('Inspection', ['inspectionPhotoGallery', 'constraintDeviceCompleteInspection']),
    ...mapState('common', [
      'templateParams',
      'doRefreshParent',
      'tabsElements',
