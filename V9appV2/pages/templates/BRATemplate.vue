...
export default {
  ...
  methods: {
    ...
    handelSearch(searchParam) {
      ...
      let seniorFilterMap = quickQueryParams.seniorFilterMap || {};
      seniorFilterMap = {
        ...this.listConfig.otherParams.seniorFilterMap,
        ...seniorFilterMap,
      };
      quickQueryParams.seniorFilterMap = seniorFilterMap;
    ...
    setPageInfo(mobileTabsBYRelation, mobileTabId, mobileTab) {
      ...
      setTimeout(() => {
        ...
        this.SET_ALLBTNS({
          ...
          comboBoxs: mobileTabsBYRelation.btnList.comboBox,
        });
      }, 100);
      //设置搜索控件参数
  ...
  onLoad(options) {
    ...
    //业务关联档案类型，如果主业务id不存在，不显示。界面提示错误
    if (!this.BusinessID) {
      this.Static_GlobalFucs.showModal(this, {
        title: this.$t('basic-modalTip'),
        content: this.$t('pageTip-noBbusinessKey'),
        showCancel: false,
      });
      return;
    }
    ...
    //定义面板信息
    const onKey = `listReload${this._uid}`;
    let tabInfo = {
      _uid: this._uid,
      templateType: 'list',
    ...
    //设置监听
    this.SET_COM_PAGERELOAD({ pageUid: onKey, type: false }); //清空面板跳转路径
    const _this = this;
    bus.$on(onKey, (params) => {
      //reload
