        ...
        >
          ...
          >
            <view slot="left" class="mInputSearch-btn" style="padding-right: 0">
              ...
              <!-- #ifndef APP-PLUS -->
              <m-button
                v-if="item.quickQuery.isDeviceScan && QRType !== '0'"
                :isFontBtn="true"
                key="searchScanBtn"
                @elementClick="deviceScan(item)"
              >
              <!-- #endif -->
                <block slot="buttonIcon">
                  <m-normal-icon icon="APP_60" :iconSize="28"></m-normal-icon>
                </block>
              </m-button>
            </view>
          </m-input-search>
...
export default {
  ...
  computed: {
    ...mapState(['bundleName', 'bundleId', 'isOriginHei', 'QRType']),
