  ...
  <view class="interlayer">
    <block v-for="(item, index) in nodes" v-bind:key="index">
      ...
      <rich-text
        ...
        :nodes="handler.getNode(item, !lazyLoad || imgLoad)"
      ...
      <view v-else-if="item.name == 'video'">
        ...
        <view
          v-if="
            (!loadVideo || item.lazyLoad) &&
            (!controls[item.attrs.id] || !controls[item.attrs.id].play)
          "
          :id="item.attrs.id"
        ...
        <video
          ...
          :src="
            controls[item.attrs.id]
              ? item.attrs.source[controls[item.attrs.id].index]
              : item.attrs.src
          "
      ...
      <!--列表-->
      <view
        v-else-if="item.name == 'li'"
        :class="item.attrs.class"
        :style="(item.attrs.style || '') + ';display:flex'"
      >
      ...
      <view
        v-else
        :id="item.attrs.id"
        :class="'_' + item.name + ' ' + (item.attrs.class || '')"
        :style="item.attrs.style"
      >
        <trees :nodes="item.children" />
      </view>
  ...
  methods: {
      ...
      if (jump) {
        ...
        if (attrs.href) {
          if (attrs.href[0] == '#') {
            ...
            // #ifdef APP-PLUS
            if (
              attrs.href.includes('.doc') ||
              attrs.href.includes('.xls') ||
              attrs.href.includes('.ppt') ||
              attrs.href.includes('.pdf')
            ) {
              uni.showLoading({
                title: _this.$t('pageTip-downloading'), //中文："文件下载中"
              });
              uni.downloadFile({
                url: attrs.href,
                success(res) {
                ...
                complete: uni.hideLoading,
						  ...
						  uni.setClipboardData({
						    ...
						    success() {
						      uni.showToast({
						        title: _this.$t('pageTip-linkCopied'), //中文："链接已复制"
						      });
						    },
						  });
            // #endif
    ...
    error(e) {
      ...
      if (target.dataset.from == 'video' || target.dataset.from == 'audio') {
        // 加载其他 source
        var index = this.controls[target.id]
          ? this.controls[target.id].index + 1
          : 1;
        if (index < target.dataset.source.length)
          this.$set(this.controls[target.id], 'index', index);
        if (target.dataset.from == 'video')
          context = uni.createVideoContext(target.id, this);
      }
      this._top &&
