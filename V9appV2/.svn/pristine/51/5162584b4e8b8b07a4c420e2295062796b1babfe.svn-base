...
Calculate.prototype = {
  ...
  operate(left, right, operator) {
    ...
    if (operator === '+') {
      ...
      if (right === 0) throw new Error('计算错误');
	...
	calRpn() {
		...
		while (this.outputQueue.length) {
		  ...
		  if (typeof token === 'number') {
			...
			if (right === null || right === undefined || right === '' || right === 'underfined' || left === null || left === undefined || left === '' || left === 'underfined') throw new Error('计算错误');
