  ...
  computed: {
    ...
    isShowCon() {
      return function (item) {
        ...
        if (item.name) {
          arr = item.name.split('.');
          type = arr[arr.length - 1].split('?')[0];
        } else {
          arr = (item.base64 && item.base64.split(';')) || [];
          type = arr[0] && arr[0].split('/')[arr[0].split('/').length - 1];
        }
        console.log('type=================================' + type);
        return 'jpg, jpeg, png'.indexOf(type?.toLocaleLowerCase()) !== -1;
      };
    },
  },
  watch: {
    base64Url(old, newvalue) {
      ...
      if (this.element.elementFormat === 2) {
        ...
        if (urlImg.indexOf('_') >= 0) {
          const index = urlImg.indexOf('_');
          id = urlImg.slice(0, index);
          url = urlImg.slice(index + 1);
          name = urlImg.slice(index + 1);
        } else url = this.base64Url;
        if (
          this.picsArray.findIndex((item) => `${item.id}` === `${id}`) === -1 &&
          url
        )
          this.picsArray = [{ id: id, url: url, name }];
      }
    },
    fileArr(v) {
      // console.log(v);
...
::v-deep {
  ...
  ::-webkit-scrollbar-thumb {
    ...
    border-width: 2px;
    background-color: rgba(157, 165, 183, 0.3);
