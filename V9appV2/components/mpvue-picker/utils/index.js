...
const transformDateToIndex = (ARG_DATE,MIN_DATE="1900/1/1") => {
	ARG_DATE = ARG_DATE || new Date()
	...
	defaultDateIndex.push(ARG_DATE.getHours()); // 时
	defaultDateIndex.push(ARG_DATE.getMinutes()); // 分
	defaultDateIndex.push(ARG_DATE.getSeconds()); // 秒
	return defaultDateIndex;
};

const getDatePickerIndex = (pickerIndex, MIN_DATE) => {
  let tempPickerIndex = [];
  // 支持传入 Date 对象
  /* eslint-disable-next-line */
  if (Object.prototype.toString.call(pickerIndex) === '[object Date]' && !isNaN(pickerIndex.getTime())) {
    tempPickerIndex = transformDateToIndex(pickerIndex,MIN_DATE);
  } else {
    if (isNaN(new Date(`${pickerIndex[0]}/${pickerIndex[1]}/${pickerIndex[2]}`).getTime())) {
      tempPickerIndex = transformDateToIndex(new Date(),MIN_DATE); // 兼容错误日期
    } else {
      tempPickerIndex = transformDateToIndex(new Date(`${pickerIndex[0]}/${pickerIndex[1]}/${pickerIndex[2]}`),MIN_DATE);
    }
  }
