<template>
  ...
  >
    ...
    >
      <view
        class="apiot-tabs-scroll-box"
        :class="{ 'apiot-tabs-scorll-flex': !isScroll }"
      >
        <view
          class="apiot-tabs-item"
...
<script>
import colorGradient from '../../utils/globalFunction/colorGradient';
...
export default {
  ...
  computed: {
    ...
    barWidthPx() {
      const { list } = this;
      let barWidth = 60;
      if (list.length > 4) barWidth = 40;
      return uni.upx2px(barWidth);
    },
    // tab的样式
    tabItemStyle() {
      return (index) => {
  ...
  methods: {
    ...
    setFinishCurrent(current) {
      // 如果滑动的索引不一致，修改tab颜色变化，因为可能会有直接点击tab的情况1
...
.apiot-tabs-scroll-box {
  position: relative;
}

.apiot-tabs-scorll-flex {
  @include vue-flex;
  justify-content: space-between;
}

.apiot-tabs-scorll-flex .apiot-tabs-item {
  flex: 1;
}
