      ...
      <slot :options="options" :data="inputSelected" :error="errorMessage">
        <view class="input-value" :class="{ 'input-value-border': border }">
          ...
          <scroll-view
            ...
            scroll-x="true"
          >
            <view class="selected-list" v-if="showType === 'all'">
              <view
                class="selected-item"
            ...
            <view class="selected-list" v-else>
              <view class="selected-item">
                <text class="text-color">{{
                  inputSelected[inputSelected.length - 1].text
                }}</text>
              </view>
            </view>
          </scroll-view>
          ...
          <view
            ...
            @click.stop="clear"
          >
            <i class="appIcon appIcon-quanshan"></i>
          </view>
          <view
            class="arrow-area"
...
export default {
  ...
  methods: {
    ...
    _processReadonly(dataList, value) {
      const isTree = dataList.findIndex((item) => item.children);
      if (isTree > -1) {
        let inputValue;
      ...
      for (let i = 0; i < value.length; i++) {
        var val = value[i];
        const item = dataList.find((v) => v.value == val);
        if (item) {
          result.push(item);
        }
      }
