  ...
  methods: {
    ...
    initFunc() {
      ...
      parser.setFunction('GET_TIME_RES', (params) => {
        ...
        // 第二个参数代表起始时间
        // 第三个参数代表天数
        // 第4个参数代表间隔类型 1 是天 2 是小时 3 是月 4 是年
        // 返回结果日期
        const start = new Date(params[1]).getTime();
        let dis = params[2] * 24 * 60 * 60 * 1000;
        let res;
        if (params[3]) {
          if (+params[3] === 2) {
            dis = params[2] * 60 * 60 * 1000;
          }
          // 月 默认按 30天
          if (+params[3] === 3) {
            dis = params[2] * 30 * 24 * 60 * 60 * 1000;
          }
          // 年默认按 365天
          if (+params[3] === 4) {
            dis = params[2] * 365 * 24 * 60 * 60 * 1000;
          }
        }
        if (+params[0] === 1) {
          res = start + dis;
        ...
        return this.$apiot.dateFormat(res, 'YYYY-MM-dd hh:mm:ss');
      });
