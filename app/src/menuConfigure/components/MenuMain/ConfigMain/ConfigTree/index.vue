...
export default {
  ...
  methods: {
    getTableName(alias) {
      const index = this.funcConfig.multiDataSource.findIndex(
        (item) => item.tableInfo.nameAlias === alias
      );
      if (index !== -1) {
        return this.funcConfig.multiDataSource[index].tableInfo.tableName;
      }
      return '';
    },
    // 下拉加载
    ...
    // 多表树查询
    async getMultiTree(loadType, params) {
      this.isLoading = true;
    ...
    getData(loadType = 'load') {
      ...
      if (this.isMultiTree) this.getMultiTree(loadType, params);
