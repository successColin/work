...
const watermarkType = ['.jpg', '.jpeg', '.png']; // 预览时需要加水印文件类型
const watermarkDownType = ['.jpg', '.jpeg', '.png', '.pdf']; // 下载时需要加水印文件类型
...
const getFileIconName = function (name) {
  ...
  if (
    ['.jpg', '.jpeg', '.png', '.gif', '.svg'].indexOf(currentFileSuffix) !== -1
  ) {
    return 'appIcon-tupianwenjian';
  }
...
const fileDownLoad = async ({ file, isWatermark = false }) => {
  ...
  try {
    ...
    let { url } = file;
    if (isWatermark && watermarkDownType.indexOf(currentFileSuffix) !== -1) {
      const serveURL = getComServeUrl();
      url = `${serveURL}system/waterMark/addWaterMark?url=${url}`;
    } else url = getComUrlByToken(url);
    const filePath = await downloadFile(url, false);
