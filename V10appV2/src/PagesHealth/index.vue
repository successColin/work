<template>
  <view class="formTemplate checkbg" style="background: #F5F5F5;">
    ...
    <apiot-navbar v-if="!hasDing" title="健康度检查"></apiot-navbar>
      ...
      <view class="health-bg">
        <view class="ricle">
          ...
          <view class="mark item">
            <span><countup :num="health" color="#FFFFFF" width='66' height='110' fontSize='110'>
              </countup></span><span class="feng">分</span>
          </view>
        </view>
        <button class="checkBtn" @click="checkHealth(false)"
          :disabled="loading" :loading="loading">一键检查</button>
      </view>
    </view>
    <view class="health-content">
      <view class="healthCheck">
        <view class="check-item halfrow" v-for="(item, index) in checkData"
          :key='index' @click="handleDeatil(index)">
            <i class="appIcon item-icon" :class="item.iconName" :color="item.color"></i>
          <view>
      ...
      <view class="healthCheck">
        <view class="check-item fullrow" v-for="(item, index) in checkDataBottom"
          :key='index' @click="handleDeatil(index + 4)">
          <view class="svgicon" :class="[item.iconPosition]"></view>
          <view class="checkLeft">
            <view class="leftcontent">
              <view>{{item.title}}</view>
              <view class="subtitle2">{{item.subTile}}
                <span style="color: #4689F5;">{{randoms[index+4]}}</span></view>
            </view>
...
export default {
  ...
  methods: {
    getRandom(isStart) { // 获随机数
      const random = [];
      for (let i = 0; i < 4; i += 1) random.push(Math.floor(Math.random() * 9));
      // 健康度
      const health = Math.floor(Math.random() * 5) + 85;
      random.push(`${health}%`);
      // 停机时长
    ...
    handleDeatil(index) { // 跳转
      ...
      uni.navigateTo({
        url: `/PagesHealth/HealthyJump/healthJump?num=${num}&name=${urlArry[index]}&index=${index}`
      });
    },
  },
  onLoad(option) {
...
.zhongjian{
  background-position: -84rpx
}
.iconright{
  background-position: -164rpx
}
.health-top{
