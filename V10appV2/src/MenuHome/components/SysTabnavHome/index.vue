...
export default {
  ...
  computed: {
    ...
    funcGroup() {
      ...
      return funcGroupArr.filter(
        (item) =>
          item.compName === 'AppInfo' || (item.permission === '1' && item.menuArr.length > 0)
      );
    },
    // 轮播图
    swiperListHasurl() {
  ...
  created() {
    ...
    this.$bus.$on('clickMenu', ({ menu, menuFilter, formData, config = {} }) => {
      this.clickMenu(menu, 1, { menuFilter, formData, ...config });
    });
  },
  beforeDestroy() {}
};
