...
const selectInspectionDo = function(params = {}, cb, cbe) {
  ...
  const sql =
    `SELECT inspectiondo.id,inspectiondo.routeCode, inspectiondo.routeName,
      ( SELECT MIN(inspectionpointdo.taskStartTime ) FROM inspectionpointdo WHERE inspectionRouteId = inspectiondo.id) AS minPlanStartDate,
      ( SELECT MAX(inspectionpointdo.taskStartTime ) FROM inspectionpointdo WHERE inspectionRouteId = inspectiondo.id) AS maxPlanStartDate,
      (SELECT COUNT(*) 
...
const selectMoList = function(inspectionDoId, cb, cbe) {
  const sql =
    ...
    count(*) AS pointCount,
    MAX(inspectionpointdo.taskStartTime) AS moMaxPlanStartDate,
    (SELECT COUNT(*)
    FROM (SELECT deviceCode,inspectionRouteId 
        FROM inspectionpointdo 
...
// 根据设备id查询巡检点列表
const selectPointDoListById = function(inspectionDoId, deviceId, cb, cbe) {
  const sql =
    `SELECT
      inspectionpointdo.inspectionPointId,
      inspectionpointdo.inspectionTaskId,
    ...
    WHERE
      inspectionpointdo.inspectionRouteId = ${inspectionDoId}
    AND inspectionpointdo.deviceId IN (${deviceId})
    AND inspectionpointdo.uploadstatus != 1
    AND inspectionpointdo.taskType = 1
    ORDER BY
      inspectionpointdo.sno IS NULL,
      inspectionpointdo.sno ASC,
      inspectionpointdo.id ASC;`;
...
// 批量更新任务状态
const BatchUpdateSql = function(inspectionTaskId, time, cb, cbe) {
  const sql = `UPDATE inspectionpointdo SET inspectCondition = 1, inspectTime = '${time}', hasAbnormal = 2, operatingstatus = 1
    WHERE inspectionpointdo.inspectionTaskId IN (${inspectionTaskId});`;
