...
export default {
  ...
  methods: {
      ...
      this.$refs.jumpForm.validate().then(async (res) => {
        ...
        try {
          const params = {
            ...
            skipMemo: this.jumpForm.skipMemo
          };
          uni.showLoading();
    ...
    checkboxChange(e) {
      const maxDateAry = [];
      this.selectRouteId = e.detail.value.map((item) => {
    ...
    selectAll(e) {
      ...
      if (e.detail.value.length) {
        ...
        const selectRouteId = this.routeData.map((item) => {
          ...
          obj.checked = true;
          data.push(obj);
          const maxDate = item.maxPlanStartDate.replace(/-/g, '/');
          const maxDateTime = new Date(maxDate).getTime();
          maxDateAry.push(maxDateTime);
          return item.id;
        });
