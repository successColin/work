...
<template>
  <view class="menuConfigure">
    ...
    <!-- #endif -->
    <apiot-menu
      v-if="menuConfig.id || menuConfig.sysMenuDesignId"
      ref="menuMain"
...
export default {
  ...
  computed: {
    menuID() {
      const { isPanel, id, menuFlag } = this.menuConfig || {};
      return isPanel ? menuFlag : id;
    },
    // 是否为首页
    isHomePage() {
      const { canShowProcressDeatail } = this.menuConfig;
      // 如果是直接跳转到流程详情的，默认顶部显示首页的样式
      if (canShowProcressDeatail) return true;
  ...
  methods: {
    ...
    forceUpdate(refreshType) {
      this.$refs.menuMain.forceUpdate(refreshType);
    },
    initPage(config) {
  ...
  onShow() {
    ...
    // 在缓存里记录当前菜单id
    if (this.menuID) {
      uni.setStorageSync('curMenuId', this.menuID);
    }
  },
