...
export default {
  ...
  computed: {
    ...
    btnsElement() {
      ...
      btns.children.forEach((btn) => {
        ...
        comp = { ...btn, ...comp };
        const { buttonStyle } = comp;
        // 自定义颜色
        // 如果按钮样式为“纯文学按钮”背景为白色，如果为“次要按钮”背景为灰色
        if (buttonStyle === 'text') {
          comp.iconColor = '#fff';
          comp.fontColor = this.$store.state.base.themeColor;
          comp.isTheme = true;
        } else if (buttonStyle === 'primary') {
          comp.fontColor = '#fff';
        } else {
          comp.iconColor = '#F1F3F6';
          comp.fontColor = '';
          comp.isTheme = true;
        }
    ...
    noMarginEL() {
      ...
      const lastObj = elementsShow[len - 1];
  ...
  methods: {
    ...
    // 点击卡片
    clickCard() {
      this.$emit('clickCard', this.value);
    },
    checkedItem() {
