...
import TreeItem from './DevicePosItem';
...
export default {
    ...
    getNewConfigData() {
      return function (data, checkList = []) {
        ...
        const currentMain = currentTab.children.find(
          (item) => item.compName === 'DevicePosTree' && item.compId === this.funcConfig.compId
        );
  ...
  methods: {
    ...
    async getData(loadType = 'load') {
      ...
      if (this.isThisLevel === 3 && list.length < 20 && this.current === 1 && this.pageFlag !== 0) {
    ...
    nodeFormat(node) {
      ...
      elementList.forEach((comp) => {
        obj[comp.dataSource.columnName] = node[comp.compId];
      });
    ...
    clickNode(node, nodeName) {
      ...
      if (getCheckFlagComp) {
        ...
        this.currentNode = this.nodeFormat(node);
