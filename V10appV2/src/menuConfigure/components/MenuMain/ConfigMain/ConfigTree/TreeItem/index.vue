  ...
  <view>
    <view v-for="(item, index) in list" :key="index" class="treeItem">
      <normal-node
        ...
        :showContent="
          item | nodeName(elementsShow, linkSymbol, multiDataSource)
        "
        :checkList="checkList"
        :canSel="
          canDesign
            ? !isMultiTree
              ? true
              : `${multiDataSource.length}` === item.dataType
            : false
        "
        :flag="getFlag"
        @checked="checkedNode"
...
export default {
  ...
  filters: {
    nodeName(value, elementsShow, linkSymbol, multiDataSource) {
      let tableName = null;
      if (multiDataSource && multiDataSource.length > 0) {
        let { dataType = 1 } = value;
        dataType = Number(dataType) - 1;
        const dataSource = multiDataSource[dataType] || {};
        const { tableInfo = '' } = dataSource;
        if (tableInfo) tableName = tableInfo.tableName;
      }
      let str = '';
      elementsShow.forEach((el) => {
        if (tableName && el.dataSource.tableName !== tableName) return;
  ...
  computed: {
    ...
    getCardIcon() {
      return function (value) {
        const { isMultiTree, multiDataSource, elements, isDictIcon } = this;
        let { icon = {} } = this;

        // 如果是多表树
        if (isMultiTree && multiDataSource.length > 0 && !isDictIcon) {
          let { dataType = 1 } = value;
          dataType = Number(dataType) - 1;
          const dataSource = multiDataSource[dataType] || {};
          const { treeIcon = {} } = dataSource;
          icon = { ...treeIcon };
        }
