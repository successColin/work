...
<template>
  <u-form-item :prop="element.compId" v-if="compShow">
    ...
    >
      ...
      <view class="elementContent" @tap.stop="clickContent">
        ...
        <template v-else>
          <span :style="{ color: contentColor }">
            {{ value | filterValue(realShowValue, element) }}</span
          >
        </template>
      </view>
    </view>
...
export default {
  ...
  computed: {
    ...
    jumpMenu() {
      ...
      if (!munuObj) return [];
      return munuObj[this.element.compId] || [];
    },
    // 跳转面板
    jumpPanel() {
      const panelObj = this.getPanel();
      if (!panelObj) return [];
      return panelObj[this.element.compId] || '';
    },
    // 数据选择框内容字体颜色
    contentColor() {
      const { jumpMenu, jumpPanel } = this;
      const { relateType, font } = this.element;

      const color = this.$store.state.base.themeColor;
      let { color: showColor } = font;

      if (relateType === 1 && jumpPanel) {
        showColor = color;
      } else if (relateType === 2 && jumpMenu.length > 0) {
        showColor = color;
      }
      return showColor;
    },
