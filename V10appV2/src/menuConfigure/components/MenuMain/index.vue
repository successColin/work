...
<template>
  ...
  >
    ...
    <process-nodes
      ...
      :isFinish="htmlConfig.isFinish"
      :approvalInfo="approvalInfo"
      :isProcessNode.sync="isProcessNode"
    ></process-nodes>
...
<script>
import { operationTriggers, selectList, selectListSingle } from '@/api/menuConfig.js';
...
import ProcessNodes from './ConfigMain/ProcessNodes';
...
export default {
        ...
        if (!result.includes(key)) {
          ...
          if (isProcess) {
            ...
            appCheckFormConfig.forEach((v) => {
              if (tab.compId === v.compId) {
                tab.notShowTab = !v.canShow;
              }
            });
          }
          if (!tab.notShowTab) {
  ...
  methods: {
    ...
    async resolveOtherApi(linkcode, actionArr) {
      ...
      if (data[triggerId]?.code && data[triggerId].code !== 200) {
        return true;
      }
    },
    // 处理返回值 获取单个值
          ...
          linkArr.forEach(async (actionArr) => {
            ...
            if (actionArr[0].triggerType === 1) {
              ...
              if (actionArr.length !== 1) {
                this.storedFun(linkcode, actionArr, resolve);
              }
            } else {
              this.storedFun(linkcode, actionArr, resolve);
            }
    ...
    // 存储过程
    async storedFun(linkcode, actionArr, resolve) {
      const state = await this.resolveOtherApi(linkcode, actionArr);
      if (state) {
        setTimeout(() => {
          resolve();
        }, 2000);
              ...
              case 4: {
                ...
                if (tab) {
                  ...
                  if ([6, 7, 15].includes(comp.compType)) {
                    ...
                    tab.form[comp.compId] = `${v.showValue || v}`;
                  }
                }
                break;
              }
    ...
    resolveFormula(flag, formulaStr, data) {
      ...
      // 获取结果值的数目
      formulaRes = formulaRes.replace(/GET_VALUE_COUNT\(\$([A-Za-z0-9]{6})\$\)/g, (v) => {
        const str = v.replace(/\$/g, "'");
        return str;
      });
      // 转换为公式
      formulaRes = formulaRes.replace(/GET_TEXT\(\$([A-Za-z0-9]{6})\$\)/g, (v) => {
        const str = v.replace(/\$/g, "'");
        return str;
      });
      // 获取表格选中列的该值
    ...
    // 获取数据选择框*的具体字段
    getSelDataColumn(compId, otherColumn) {
      const comp = this.getAllPane.compObj[compId];
      const compArry = [6, 7, 15];
      if (compArry.indexOf(comp.compType) === -1 || comp.selDataColumnStr) return;
      const { triggerMap } = this.configDataArr;
      const initObj = triggerMap[1] && triggerMap[1][compId];
      const changeObj = triggerMap[2] && triggerMap[2][compId]; // 值变化
      // 同时需要兼容初始化，和值变化
      const arry = [];
      if (initObj) arry.push(initObj);
      if (changeObj) arry.push(changeObj);
      const columnArr = ['id'];
      if (otherColumn) columnArr.push(otherColumn);
      if (arry.length > 0) {
        arry.forEach((obj) => {
      ...
      comp.selDataColumnStr = [...new Set(columnArr)].join(',');
      return comp.selDataColumnStr;
    },
    // 触发器-值变化
    ...
    initFunc() {
      ...
      parser.setFunction('GET_SELDATA', (params) => {
        ...
        console.log(comp.name, comp.selData, params);
