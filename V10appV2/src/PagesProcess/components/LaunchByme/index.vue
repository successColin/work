...
<template>
  <view class="launchByme" v-if="computedHeight > 0">
    ...
    <view :style="{ height: `${computedHeightList2}px` }">
      <apiot-toast
      ...
      <apiot-list-nodata
        ...
        :style="{ height: computedHeightList2 + 'px' }"
      ></apiot-list-nodata>
    </view>
...
export default {
  ...
  methods: {
    ...
    clickFilter() {
      ...
      uni.navigateTo({
        url: `/PagesProcess/components/AdvancedScreen/index?processNameArr=${this.processNameArr}&compid=launchByme`
      });
    },
    handleSearch(params) {
    ...
    async clickNode(node) {
      ...
      uni.navigateTo({
        url: `/menuConfigure/index?id=${appPanelId}&title=${instanceName}&triggerUserName=${triggerUserName}&isProcess=true&workflowDataId=${dataId}&instanceId=${instanceId}&processNodeId=${nodeId}&type=${this.selectStatus}&triggerUserId=${triggerUserId}&taskType=${taskType}&sno=4`,
  ...
  created() {
    // 设置监听，用于转审时，刷新列表
    this.$bus.$on('PROCESS_REFRESH_4', (v) => {
  ...
  beforeDestroy() {
    this.$bus.$off('PROCESS_REFRESH_4');
  }
};
</script>
