...
<script>
import { getCcTomeList, getNodeAttr } from '@/api/pagesProcess.js';
...
export default {
  ...
  methods: {
    ...
    clickFilter() {
      ...
      uni.navigateTo({
        url: `/PagesProcess/components/AdvancedScreen/index?processNameArr=${this.processNameArr}&compid=ccTom`
      });
    },
    handleSearch(params) {
    ...
    async clickNode(node) {
      // 跳转至审批详情
      const { appPanelId, instanceName, instanceId, dataId, nodeId, triggerUserId } = node;
      if (!appPanelId) return;
      const { processConfigs } = this.$store.state.process;
      if (!processConfigs[nodeId]) await this.getNodeAttr(nodeId);

      uni.navigateTo({
        url: `/menuConfigure/index?id=${appPanelId}&title=${instanceName}&triggerUserName=${triggerUserName}&isProcess=true&workflowDataId=${dataId}&instanceId=${instanceId}&processNodeId=${nodeId}&isFinish=1&isCcTom=1&type=CC&triggerUserId=${triggerUserId}&sno=3`,
    ...
    async getData(loadType = '') {
      ...
      try {
        ...
        const result = await getCcTomeList(params);
  ...
  created() {
    // 设置监听，用于转审时，刷新列表
    this.$bus.$on('PROCESS_REFRESH_3', (v) => {
  ...
  beforeDestroy() {
    this.$bus.$off('PROCESS_REFRESH_3');
  }
};
</script>
