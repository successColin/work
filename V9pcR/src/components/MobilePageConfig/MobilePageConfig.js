 ...
 * @Last Modified time: 2019-12-13 15:26:42
 * @Desc: 界面配置（预设界面）
 */
...
import {
  getMobileTabTree,
  insertMobileTabGroup,
  insertMobileTab,
  updateMobileTabGroup,
  updateMobileTab,
} from '@/services/mobileConfig';
import { getNodeDataFromTreeData, setNewActionType } from '@/utils/common';
import styles from './MobilePageConfig.less';

@connect(state => ({
  functionData: state.tabs.activeTabData.functionData,
}))
class MobilePageConfig extends Component {
  ...
  fetchTree = (resData) => {
    ...
    getMobileTabTree({ keywords }).then(treeData => {
...
export default Form.create()(MobilePageConfig);
