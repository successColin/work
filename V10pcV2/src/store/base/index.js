  ...
  if (module.children) {
    module.children.forEach((menu) => {
      ...
      if (+menu.menuLevel === 2) {
        ...
        } else if (menu.routeName === 'ApiotMenu') {
          menu.path = `/menu/${menu.id}?title=${menu.menuName}`;
        } else {
...
export default {
  ...
  mutations: {
    ...
    changeApprovalNum(state) {
      state.approvalChangeNum += 1;
    },
    changeRouteAuthArr(state, arr) {
      state.routeAuthArr = arr;
    },
    changeVerificationResults(state, v) {
      state.flag = v;
    },
  ...
  actions: {
    ...
    async getHomeRoute({ commit }) {
      try {
        ...
        const arr = res.filter((item) => item.enabled);
        const newRes = arr.map((item) => {
    ...
    async setApprovalNum({ commit }) {
      commit('changeApprovalNum');
    },
    async setVerificationResults({ commit }, payload) {
      commit('changeVerificationResults', payload);
    },
