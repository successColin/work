...
<template>
  <div class="GeneralTableWrap">
    ...
    <div class="seamless-warp" :style="getListScroll" ref="listScroll">
      <div
      ...
      >
        ...
        >
          <div
            ...
            :class="{
              ellipsis: column.isEllipsis,
              unEllipsis: !column.isEllipsis,
            }"
          >
            <img
              class="prefix"
              v-if="getPrefixUrl(column, item)"
              :src="getRealUrl(column, item)"
            ...
            <span
              ...
              v-if="
                column.interactionMode === 4 &&
                getFilesType(column, item).isShow
              "
...
<script>
import { Decrypt, Encrypt, formatDate, getBlob, isJiami, saveAs } from '@/utils/utils';
...
export default {
  ...
  computed: {
    panelObj() {
      const { activeObj, tabPaneConfig } = this.panelConfig;
      return {
        ...tabPaneConfig,
        ...activeObj,
        panelVarObj: {},
    ...
    getFilesType() {
      ...
      return function (column, data) {
        ...
        if (!arr.length) return { isShow: false };
        const type = arr.at(-1);
        if (!type) return { isShow: false };
        return {
          isShow: true,
    ...
    getIconStyles() {
      ...
      return function (column, i, data) {
        const { fieldFontSize, fieldColor } = column;
        const {
    ...
    getTheadTotalWidth() { // 获取列表宽总宽度
      const { columnArr } = this;
      const total = columnArr.reduce((pre, current) => {
    ...
    getColumnStyles() {
      return function (obj) {
        ...
        const {
          stylesObj: { theadFontFamily, theadFontWeight, theadFontSize, theadColor, theadTextAlign }
        } = this.config;
        const realWidth = this.getColumnWidth(fieldWidth);
    ...
    getLiStyles() {
      return function ({ snoZ }, i) {
        ...
        if (
          ...
          speHighlightArr.length &&
          !autoHighlight &&
          speHighlightArr.includes(`${snoZ}`)
        ) {
          bg = `background-color:${highlightBgColor} !important;`;
          highLightStyles = `color: ${highlightColor} !important;fontSize:${highlightFontSize}px;fontWeight:${highlightFontWeight};fontFamily:${highlightFontFamily};`;
        } else {
        ...
        return this.transactionY
          ? `${styles}transition: transform 500ms ease-in 0.5s;`
          : `${styles}`;
      };
    },
    getSnoStyles() {
      return function (i) {
        const {
          stylesObj: { snoFontFamily, snoFontWeight, snoFontSize, snoColor }
        } = this.config;
        const {
          stylesObj: {
            rows = 10,
    ...
    getColumnContentStyles() { // 是否允许点击
      return function (column, data) {
        ...
        if ((interactionMode === 2 && paneConfig.tabPaneConfig?.id) || (interactionMode === 3 && skipMenuConfig.length)) {
      ...
      return function (column, item) {
        if (this.$route.name === 'appCustomPage') {
          ...
          if (hasOwnProperty.call(query, 'CSMToken') && query.CSMToken) {
            ...
            return this.$parseImgUrl(this.getPrefixUrl(column, item), query.CSMToken);
          }
          return '';
        }
        return this.$parseImgUrl(this.getPrefixUrl(column, item));
      };
    }
  },
  ...
  methods: {
    ...
    handleImagePreview(column, item) {
      // 图片预览
      if (this.type === 'design') {
        return;
      }
    ...
    doHref(config, obj) {
      ...
      if (interactionMode === 2) {
        const lastPaneConfig = cloneDeep(paneConfig);
    ...
    doSpringPanel(panelConfig = {}) {
      const { tabPaneConfig: curPaneObj, activeObj: { dialogTitle } } = panelConfig;
    ...
    move() {
      // 数据开始移动
      const {
        stylesObj: { rows = 10, loop, loopTime, animationTypes }
      } = this.config;
      // , animationTypes
      if (!loop) {
        return;
