...
export default {
  ...
  methods: {
    ...
    selectAll(selection) {
      if (!selection) {
        const data = JSON.parse(JSON.stringify(this.curData));
        const resArr = [];
        this.multiArr.forEach((item) => {
          const index = data.findIndex(
            (v) => v[this.rowKey] === item[this.rowKey],
          );
          if (index !== -1) {
            data.splice(index, 1);
          } else {
        ...
        this.multiArr = resArr;
        this.$emit('select-all', this.multiArr);
    ...
    handleSelectionChange(val) {
      ...
      if (this.reserveSelection) {
        if (!this.jump) {
          val.forEach((v) => {
            const index = this.multiArr.findIndex(
              (item) => item[this.rowKey] === v[this.rowKey],
            );
            if (index === -1) {
    ...
    rowDrop() {
      const tbody = document.querySelector(
        `${this.dropClass} .el-table__body-wrapper tbody`,
      );
      const _this = this;
    ...
    columnDrop() {
      const wrapperTr = document.querySelector(
        `${this.dropClass} .el-table__header-wrapper tr`,
      );
