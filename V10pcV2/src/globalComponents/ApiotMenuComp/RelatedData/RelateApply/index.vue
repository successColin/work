...
<template>
  ...
  >
    ...
    <div
      class="contentWrap__content"
      :class="[{ isHidden: isHidden }, { hasTitle: configData.showTitle }]"
    >
      <div class="btnWrap">
        <apiot-button
          :style="getStyle"
        ...
        >
          <span
            class="iconfont icon-xinzengwenjianjia btnWrap__btnCommon"
          ></span>
      ...
      <div class="contentTypeWrap">
        <block-content
          ...
          v-if="
            active === 1 && showType !== 3 && ![7, 8, 10].includes(groupType)
          "
...
export default {
  ...
  computed: {
    getHeightStyle() {
      let style = '';
      if (this.configData.showHeight && this.configData.heightPix) {
    ...
    showReplaceBtn() {
      if (
        this.configData.showReplaceBtn &&
        this.canShowBtn &&
  ...
  beforeDestroy() {
    this.$bus.$off(`reloadArea${this.onlyFlag()}`, this.reloadArea);
  ...
  methods: {
    ...
    initTable() {
      ...
      if (this.configData.relateBusiComp) {
        this.relateBusiComp.value =
          this.getAllForm()[this.configData.relateBusiComp.compId];
        this.relateBusiComp.tableName =
          this.configData.relateBusiComp.tableName;
      }
    ...
    async handlePreview(item, flag = true) {
      console.log(item);
      ...
      if (treeType === 3) {
        ...
        if (flag) {
          ...
          this.picList = [item].filter((liObj) => {
            const { treeType: imageType } = liObj.sysKlTree || liObj;
            return imageType === 3;
          });
        }
    ...
    getFileUrl(obj) {
      ...
      if (
        ...
        if (obj.sysKlTree && obj.sysKlTree.url) {
          url = obj.sysKlTree.url.startsWith('blob:')
            ? obj.sysKlTree.name
            : obj.sysKlTree.url;
        } else {
          url = obj.url.startsWith('blob:') ? obj.name : obj.url;
        }
        if (!url) return xls;
