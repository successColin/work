    ...
    <section class="hisBody">
      ...
      <ul class="hisBody__list">
        ...
        >
          ...
          >
            <img
              class="hisBody__item--img"
              :src="fileUrl(item)"
              alt=""
              @click.stop="preView(item)"
            />
            <span class="hisBody__item--fileName" @click.stop="preView(item)">{{
...
export default {
  ...
  computed: {
    ...
    renderAvtor() {
      return (obj) => {
        let avtor = obj.uploadUserName
          ? cnchar.spell(obj.uploadUserName, 'first', 'up')
          : '';
  ...
  methods: {
    preView(item) {
      this.$emit('preview', { sysKlTree: item }, false);
    },
    calculateSize(size) {
      const kb = 1024;
      const mb = kb * 1024;
