...
<script>
import { listDeviceTree, singleSave } from '@/api/menuConfig';
...
export default {
  ...
  methods: {
    ...
    async singleSave(type, draggingNode, dropNode) {
      ...
      let formInfo = [];
      if (
        +draggingNode.data.dataType === 1 &&
        +tempDropNode.data.dataType === 1
      ) {
        formInfo = [
          {
            columnName: 'id',
      ...
      const params = {
        ...
        removeFileIds: [],
        tableName:
          +draggingNode.data.dataType === 1 ? 'sys_position' : 'sys_device',
