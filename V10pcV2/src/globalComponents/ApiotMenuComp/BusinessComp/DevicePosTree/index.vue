    ...
    >
      <div class="menuMain__cardRight" v-loading="loading">
        ...
        <div
          ...
          v-if="isPreview"
        >
          <el-tree :data="previewData"></el-tree>
        </div>
        <!-- 配置页 -->
        <draggable
          v-else-if="isConfig"
  ...
  methods: {
    ...
    getFilterParams() {
      ...
      this.configData.termParams = `${
        this.configData.deviceFilter
          ? this.configData.deviceFilter.termParams
          : ''
      },${
        this.configData.positionFilter
          ? this.configData.positionFilter.termParams
          : ''
      }`;
    ...
    getCurAreaTerm(filterArr, tableName) {
      if (filterArr) {
        const obj = filterArr.find(
          (filter) =>
            filter.compId === this.configData.compId &&
            tableName === filter.tableInfo.tableName,
        );
        return obj;
      }
      return '';
      ...
      if (searchInfo && flag) {
        ...
        if (this.getFatherPanel && !this.getFatherPanel()) {
          if (+this.$route.query.isJump === 1) {
            ...
            const positionFilter = this.getCurAreaTerm(
              menuFilter,
              'sys_position',
            );
            if (positionFilter) {
        ...
        const params = {
          ...
          sysMenuDesignId: this.sysMenuDesignId(),
          parentId,
          isThisLevel: this.$store.state.userCenter.userInfo.roleIds.includes(1)
            ? 1
            : 2,
          dataType,
          deviceDesignRegionalTabInfoVO: this.configData.deviceFilter,
        ...
        if (this.getFatherPanel && !this.getFatherPanel()) {
          if (+this.$route.query.isJump === 1) {
            ...
            if (deviceFilter && !positionFilter) {
              params.dataType = 2;
            }
            if (deviceFilter || positionFilter) {
              params.parentId = '';
    ...
    getListParams(node, callBack) {
      ...
      if (this.getFatherPanel && !this.getFatherPanel()) {
        if (+this.$route.query.isJump === 1) {
          ...
          if (deviceFilter && deviceFilter.enableChildSearch === 1) {
            params.deviceDesignRegionalTabInfoVO = deviceFilter;
          }
          const positionFilter = this.getCurAreaTerm(
            menuFilter,
          ...
          if (positionFilter && positionFilter.enableChildSearch === 1) {
            params.positionDesignRegionalTabInfoVO = positionFilter;
          }
        }
      }
