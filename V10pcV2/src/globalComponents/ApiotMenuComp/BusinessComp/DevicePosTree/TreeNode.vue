...
export default {
  ...
  computed: {
    ...
    showAddDevice() {
      if (
        !this.$store.getters
          .getMenuResource(this.$route.params.id || this.$route.query.menuId)
          .includes(2)
      ) {
        return false;
      }
  ...
  methods: {
    ...
    addPos() {
      ...
      this.panelObj = this.resolveFilterVar(
        this.getPanel()[this.configData.posAddPos],
      );
    ...
    addDevice(dataType) {
      ...
      if (dataType === 1) {
        this.panelObj = this.resolveFilterVar(
          this.getPanel()[this.configData.posAddDevice],
        );
      } else {
        this.panelObj = this.resolveFilterVar(
          this.getPanel()[this.configData.deviceAddDevice],
        );
      }
