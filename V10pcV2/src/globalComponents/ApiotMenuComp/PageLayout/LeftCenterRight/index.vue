<template>
  ...
  >
    ...
    <div class="LeftCenterRight__center" ref="resize" v-if="!isConfig"></div>
    <div
      class="LeftCenterRight__center"
      :style="`flex:${configData.centerWidth}`"
      ref="center"
    >
      <tab-main
...
export default {
  ...
  methods: {
    ...
    dragControllerDivRight() {
      const { resizeRight } = this.$refs;
      const { left } = this.$refs;
      const { right } = this.$refs;
      const leftWidth = left.offsetWidth;
      const rightWidth = right.offsetWidth;
      const allWidth = this.$refs.box.offsetWidth * 0.05;
      const max = rightWidth + allWidth;
      const min = rightWidth - allWidth < 170 ? 170 : rightWidth - allWidth;
      const mid = this.$refs.center;
      // const { box } = this.$refs;
      // console.log(max, min);
      // 鼠标按下事件
      resizeRight.onmousedown = function (event) {
        const startX = event.clientX;
        resizeRight.right = resizeRight.offsetRight;
        // 鼠标拖动事件
        document.onmousemove = function (e) {
          const endX = e.clientX;
          // （endx-startx）=移动的距离。resize[i].left+移动的距离=左边区域最后的宽度
          let moveLen = rightWidth - (endX - startX);

          if (moveLen < min) moveLen = min; // 左边区域的最小宽度为150px
          if (moveLen > max) moveLen = max; // 右边区域最小宽度为150px

          resizeRight.style.right = moveLen; // 设置左侧区域的宽度

          left.style.flex = `0 0 ${leftWidth}px`;
          right.style.flex = `0 0 ${moveLen}px`;
          mid.style.flex = '1';
        };
        // 鼠标松开事件
        document.onmouseup = function () {
        ...
        if (resizeRight.setCapture) {
          resizeRight.setCapture(); // 该函数在属于当前线程的指定窗口里设置鼠标捕获
        }
