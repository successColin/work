      ...
      >
        ...
        >
          ...
          <div
            ...
            :id="item.compId"
            :class="[
              { firstIsTable: firstIsTable(item) },
              { isOnlyOneChild: isOnlyOneChild(item) },
            ]"
  ...
  computed: {
    ...
    getTabs() {
      if (this.isPreview) {
        ...
        return this.configData.children.filter((item) => !item.notShowTab);
      }
    ...
    getCurTab: {
      get() {
        if (this.isPreview) {
          // return this.configData.curCompId;
          return this.getTabs[0].compId;
        }
    ...
    isOnlyOneChild() {
      return (item) => {
        ...
        item.children.forEach((area) => {
          if (area.compName === 'MenuMain') {
            let arr = [];
            const index = area.children.findIndex(
              (child) => child.areaType === 1,
            );
            if (index !== -1 && this.configData.children[index]) {
              arr = area.children[index].children;
            }
            const index1 = arr.findIndex((v) => v.canShow);
            if (index1 === -1) {
              len -= 1;
            }
          }
  ...
  mounted() {
    ...
    if (!this.isConfig) {
      ...
      if (
        ...
        this.$bus.$on(`tabHidden${this.onlyFlag()}`, this.tabHidden);
      } else {
        this.tabsArr = this.configData.children.filter(
          (item) => !item.notShowTab,
        );

        const index = this.tabsArr.findIndex(
          (tab) => tab.compId === this.configData.firstShowTabId,
        );
        if (index === -1 && this.tabsArr.length) {
      ...
      this.$bus.$on(`returnFirst${this.onlyFlag()}`, this.returnFirst);
    }
  },
  beforeDestroy() {
    this.$bus.$off(`tabHidden${this.onlyFlag()}`, this.tabHidden);
    this.$bus.$off(`returnFirst${this.onlyFlag()}`, this.returnFirst);
  },
  methods: {
    // 回到第一个
    returnFirst() {
    ...
    tabHidden() {
      ...
      this.configData.children
        .filter((item) => !item.notShowTab)
        .forEach((tab) => {
          // 不存在需要隐藏的数组中
    ...
    getMoreOperate(item) {
      ...
      if (length - getBtnsArr.rightIndex - 4 > 0) {
        moreOperateArr = getBtnsArr.children.splice(
          4,
          length - getBtnsArr.rightIndex - 4,
          {
