    ...
    <div class="menuMain__content" :class="[{ isHidden: isHidden }]">
      ...
      >
        <div
          ...
          :class="[{ isMain: !this.isConfig && this.getIsMain() }]"
        >
          <!-- 配置页 -->
          ...
          >
            ...
            >
              <component
                ...
                :multiEntityArr="multiEntityArr"
                :showSinglebox="numAndSel.showRadio && index === 0"
                :showCheckbox="numAndSel.showSelection && index === 0"
        ...
        <!-- <apiot-pagination
          v-if="!isConfig && configData.hasPagination && showPagi && false"
          :isSystem="false"
...
export default {
  ...
  data() {
    return {
      ...
      isHidden: false,
      timer: null,
      loadNodeMap: new Map(), // 展开节点的map
      allDataMap: new Map(), // 所有数据的map
    };
  },
  ...
  methods: {
    ...
    // 更新该区域
    reloadArea(areaArr, compId, filterObj, isAdd, isTableBtn) {
      // 按钮全部刷新
      if (areaArr === 'all') {
        if (!this.getNotInitArr().includes(this.configData.compId)) {
          this.multiEntityArr = [];
      ...
      if (
        ...
        this.configData.compName === 'TreeTable'
      ) {
      ...
      // 树表的更新
      if (areaArr === 'treeTableUpdate') {
        if (filterObj === 1) {
          this.treeSave(isAdd, isTableBtn);
        } else if (filterObj === 2) {
          this.treeDelete();
        }
      }
    ...
    async treeSave(isAdd, isTableBtn) {
      ...
      if (!isAdd) {
        ...
        } else if (this.multiEntityArr.length) {
          // 不是表格行内的编辑
    ...
    treeDelete() {
      ...
      const fatherUniqueArr = [];
      this.multiEntityArr.forEach((item) => {
        fatherUniqueArr.push(item.fatherUnique);
      });
      fatherUniqueArr.forEach(async (unique) => {
        if (this.loadNodeMap.has(unique)) {
          const { treeData, resolve } = this.loadNodeMap.get(unique);
          const data = await this.getCurDataList(treeData);
          resolve(this.resolveData(data, treeData.unique));
    ...
    async getCurDataList(treeData) {
      ...
      const params = {
        ...
        isThisLevel: 2,
        nodeId: treeData[this.getIdCompId],
      };
    ...
    async load(treeData, node, resolve) {
      const data = await this.getCurDataList(treeData);
      this.loadNodeMap.set(treeData.unique, { treeData, node, resolve });
      resolve(this.resolveData(data, treeData.unique));
    },
    resolveData(data, fatherUnique) {
      const keys = this.getFeatureArr.form
        ? Object.keys(this.getFeatureArr.form)
        : [];
      if (!data) {
      ...
      data.forEach((form) => {
        // unique 代表唯一值
        // notChange 代表没改变
        form.unique = this.configData.searchInfo
          ? createUnique()
        ...
        form.fatherUnique = fatherUnique;
        this.allDataMap.set(form.unique, form);
      });
      return data;
    },
    // 获取列表数据
    async getSidebarList() {
      ...
      this.tableData = this.resolveData(data, 0);
      this.loading = false;
      ...
      this.$nextTick(() => {
        // console.log(document.querySelector('.el-table__expand-icon'));
        document
          .querySelector(`.${this.configData.compId} .el-table__expand-icon`)
          .firstChild.click();
      });
    },
    ...
    sortChange(column) {
      ...
      const obj = this.dropColumnData.find((data) => {
        if (
          data.name === column.column.label &&
          data.dataSource.columnName === column.prop
        ) {
          return true;
        }
        return false;
      ...
      if (
        obj &&
        obj.dataSource.mainTableInfo &&
        obj.dataSource.mainTableInfo.tableName
      ) {
