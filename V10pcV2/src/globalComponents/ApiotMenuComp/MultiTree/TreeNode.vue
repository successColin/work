  ...
  <div class="treeNode" @mouseenter="mouseenter" @mouseleave="mouseleave">
    <div class="treeNode__content">
      ...
      <div
        ...
        :class="[{ isSelect: showCheckbox || showSinglebox }]"
        :id="data.treeId"
...
export default {
  ...
  computed: {
    ...
    getCurBtns() {
      const obj = this.getBtnsArr.find(
        (item) => item.dataSource.alias === this.getCurTableAlias,
      );
      if (obj) {
        return obj;
      }
        ...
        if (tempStr) {
          if (item.enableDict) {
            const dictArr = this.$store.getters.getCurDict(
              item.dataSource.dictObj.dictKey,
            );
