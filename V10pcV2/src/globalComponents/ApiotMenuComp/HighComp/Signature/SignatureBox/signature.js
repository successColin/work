...
class Handwriting {
  ...
  init() {
    if (this.canvasName) {
      ...
      // 画笔
    } else this.canvasDom = document.createElement('canvas');
  ...
  async setImg(url) {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        this.canvasDom.width = img.width;
        this.canvasDom.height = img.height;
        this.ctx = this.canvasDom.getContext('2d');
  ...
  watermark(config) {
    ...
    // this.ctx.strokeRect(10, 10, 120, 80);

    // 在x轴为120，y轴为120的位置画长为160，宽为100的填充长方形
    // this.ctx.fillRect(120, 120, 160, 100);
    // this.ctx.draw(true);
  }
  ...
  uploadScaleStart(event) {
    ...
    this.ctx.fillStyle = this.lineColor; // 初始线条设置颜色
    // this.ctx.globalAlpha = this.transparent; // 设置半透明
  ...
  uploadScaleMove(event) {
    ...
    this.pointToLine(this.currentLine);
    if (this.canvasHeight - point.y <= 5 || this.canvasWidth - point.x <= 5) {
      this.uploadScaleEnd(event, true);
    }
  }

  // 笔迹结束
  uploadScaleEnd(event, isOut = false) {
    this.ctx.closePath(); // 收笔
    const e = event;
    const point = {
      x: e.offsetX,
      y: e.offsetY,
    };
    if (e.type !== 'mouseup' && !isOut) return 0;
    this.lastPoint = this.currentPoint;
    this.currentPoint = point;
    this.currentLine.unshift({
      time: new Date().getTime(),
  ...
  bethelDraw(point, is_fill, color) {
    ...
    // this.ctx.beginPath();
    // this.ctx.moveTo(237.7, 138);
    // this.ctx.bezierCurveTo(232.2, 139.9, 235.9, 136.2, 237.7, 138);
    // this.ctx.stroke();
  ...
  dataURLtoFile() {
    const dataurl = this.canvasDom.toDataURL('image/png');
