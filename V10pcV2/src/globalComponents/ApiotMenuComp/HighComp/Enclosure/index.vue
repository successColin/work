  ...
  >
    ...
    >
      ...
      <el-upload
        class="upload"
        :class="[
          { isDisabled: !isConfig && configData.canReadonly },
        ...
        :file-list="fileList"
        :disabled="configData.canReadonly || isPreview"
      >
        ...
        >
          <img
            ...
            :class="[
              {
                canPreview: !['else'].includes(getCurIconName(file.name)),
              },
            ]"
...
export default {
  ...
  computed: {
    isMaxLength() {
      if (this.configData.maxFileCount) {
        return this.configData.maxFileCount === this.fileList.length;
      }
      return false;
    },
  },
  ...
  methods: {
    ...
    async preview(file) {
      ...
      // 配置了文件服务器
      if (
        ['XLS', 'TXT', 'PPT', 'DOC', 'zipFile', 'PDF'].includes(
          this.getCurIconName(file.name),
        ) &&
