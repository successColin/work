    ...
    pickerOptions() {
      const obj = {
        disabledDate: (time) => {
          ...
          if (minTime) {
            minTime = new Date(`${formatDate(new Date(minTime), 'yyyy-MM-dd')} 00:00:00`).getTime();
          }
          if (maxTime) {
            maxTime = new Date(`${formatDate(new Date(maxTime), 'yyyy-MM-dd')} 00:00:00`).getTime();
          }
          if (minTime && maxTime) {
  ...
  methods: {
    handleChangePicker(v) {
      if (v instanceof Array) {
        ...
        const curV = new Date(v).getTime();
        if (minTime && curV < minTime) {
          this.parent.form[this.configData.compId] = formatDate(
            new Date(minTime),
            'yyyy-MM-dd hh:mm:ss'
          );
        }
        if (maxTime && curV > maxTime) {
          this.parent.form[this.configData.compId] = formatDate(
            new Date(maxTime),
            'yyyy-MM-dd hh:mm:ss'
          );
