      ...
      >
        ...
        >
          <i
            ...
            :class="[{ ' m-r-4': [1, 2].includes(configData.highStyle) }]"
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <span
                        class="searchCondition__his--value searchCondition__his--tags"
                        v-if="row.showValue"
                      >
                        <apiot-tag
                          ref="tag"
...
export default {
  ...
  mounted() {
    if (!this.isConfig) {
      ...
      this.$bus.$on(`clearSearch${this.onlyFlag()}`, this.clearSearch);
  ...
  methods: {
    ...
    initHighForm() {
      ...
      if (
        this.configData.searchCompArr &&
        this.configData.searchCompArr.length !== 0
      ) {
        const arr = [];
        const idsArr = [];
        this.configData.searchCompArr.forEach((comp) => {
          ...
          if (
            curComp &&
            curComp.canShow &&
            !idsArr.includes(comp.value) &&
            ![7, 12, 22, 23, 34].includes(comp.compType)
          ) {
    ...
    // 找到对应的组件
    getComp(compId) {
      const index = this.featureArr.children.findIndex(
    ...
    createColumn(comp, flag) {
      ...
      // 如果是label 转的数据单选框 或者是数据单选框 但是他的字典没开启
      if (comp.compType === 6 && !comp.enableDict) {
    ...
    clearSearch(tableCompId, flag) {
      if (this.tableInfo.compId === tableCompId) {
        this.clearHigh(flag);
      }
    },
    // 清空高级搜索
    ...
    hisClick(his) {
      ...
      searchContent.columnsInfo.reduceRight((total, column, index) => {
        if (column.compId) {
          const compId =
            column.compId.indexOf('_') === -1
              ? column.compId
              : column.compId.slice(0, -1);
  ...
  beforeDestroy() {
    ...
    this.$bus.$off(`clearSearch${this.onlyFlag()}`, this.clearSearch);
