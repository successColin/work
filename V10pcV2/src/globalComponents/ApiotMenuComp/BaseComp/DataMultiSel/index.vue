<template>
  ...
  >
    ...
    >
      ...
      ></apiot-tag> -->
      <ApiotTagBox
        v-if="parent.form[this.configData.compId]"
...
export default {
  ...
  methods: {
    ...
    showPanelDialog() {
      ...
      // if (this.panelObj.pageConfig[0].designOverallLayout[0].enabledPrototype) {
      //   return this.$message({
      //     type: 'warning',
      ...
      if (this.panelObj && this.panelObj.panelName) {
        ...
        console.log(this.parent.form[this.configData.compId], obj);
    ...
    setDataSel(arr) {
      ...
      arr.forEach((item) => {
        str += `${item.id},`;
        if (item[this.configData.multiTable.column.columnName]) {
          str1 += `${item[this.configData.multiTable.column.columnName]},`;
        }
      });
      if (this.parent.form[this.configData.compId] !== str.slice(0, -1)) {
        this.$set(this.parent.form, this.configData.compId, str.slice(0, -1));
        this.$set(
          this.parent.form,
          `${this.configData.compId}_`,
          str1.slice(0, -1),
        );
      }
