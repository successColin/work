    ...
    >
      <section
        ...
        :class="[
          ...
          { hasPagination: !isPreview && configData.hasPagination },
          { isApp: $route.query.isApp == 1 },
        ]"
        configData="功能区"
      >
        <div
          ...
          v-for="(item, index) in isConfig || isPreview ? [{}] : sidebarData"
        ...
        <div
          class="apiotNoData1"
          v-show="!isConfig && !isPreview && sidebarData && !sidebarData.length"
        ></div>
      </section>
      <apiot-pagination
        v-if="!isConfig && !isPreview && configData.hasPagination"
  ...
  methods: {
    ...
    initHeight() {
      if (!this.isConfig) {
        ...
        if (tableMaxHeight > 0) {
          this.cardTableStyle = `height: ${tableMaxHeight}px`;
          return;
        }
        this.cardTableStyle = 'height: 100%';
        return;
      }
      this.cardTableStyle = '';
    },
    ...
    async getSidebarList() {
      ...
      this.$nextTick(() => {
        if (this.$refs.menuMainCard && this.menuMainCardHeight === 0) {
          this.menuMainCardHeight =
            this.$refs.menuMainCard[0].offsetHeight - 24;
        }
      });
      ...
      this.$emit(
        'showRight',
        this.sidebarData ? this.sidebarData.length !== 0 : false,
      );
      if (data && data.length) {
