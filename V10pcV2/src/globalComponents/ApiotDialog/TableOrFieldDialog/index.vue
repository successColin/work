...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch(
      'getCurrentDict',
      'TABLES_TYPE,COLUMNS_TYPE,TEMPLATE_TYPE',
    );
    if (this.dialogType === 3) {
  ...
  methods: {
    ...
    async getList() {
      if (this.tableArr.length) {
        this.tableData = this.tableArr.filter((item) => {
          ...
          const index = keys.findIndex((key) => {
            if (
              item[key].toString().indexOf(this.dictKeywords) !== -1 ||
              !this.dictKeywords
            ) {
              return true;
            }
      ...
      if (this.dialogType === 1) {
        ...
        if (this.notShowSys) {
          res = res.filter((item) => {
            if (
              item.columnName === 'root_path' ||
              item.columnName === 'parent_id'
            ) {
              return false;
            }
        ...
        if (!this.idNeedId) {
          this.filterColumn.push('id');
        }
        const arr = [];
        res.forEach((item) => {
          if (!this.filterColumn.includes(item.columnName)) {
            arr.push(item);
          }
        });
        this.tableData = arr;
        ...
        const params = {
          ...
          clientType:
            this.$route.query.isApp === '1' || this.isApp === 1 ? 2 : 1,
          ...this.otherParams,
        };
    ...
    async setSelectPanelArr() {
      if (
        this.$store.state.dictManage.PANEL_TYPE
          ? !this.$store.state.dictManage.PANEL_TYPE
          : false
      ) {
