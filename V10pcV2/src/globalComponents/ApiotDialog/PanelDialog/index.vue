...
export default {
  ...
  computed: {
      ...
      if (this.panelObj) {
        if (this.panelObj.pageConfig) {
          if (this.panelObj.pageConfig[0].designOverallLayout) {
            if (
              [3, 5].includes(
                this.panelObj.pageConfig[0].designOverallLayout[0].selShowType,
              )
            ) {
              return true;
            }
    ...
    getSize() {
      if (this.panelObj && this.panelObj.panelSize) {
        return +this.panelObj.panelSize;
      }
      return 2;
    },
  },
  methods: {
    ...
    getForm() {
      ...
      this.multiArr.forEach((item) => {
        // const key1 =
        if (
          item[this.dataSelObj.name] ||
          item[this.getMapKey(this.dataSelObj.name)]
        ) {
          valueArr.push(
            item[this.dataSelObj.name] ||
              item[this.getMapKey(this.dataSelObj.name)],
  ...
  watch: {
    ...
    showPanel(v) {
      if (v) {
        this.$nextTick(() => {
          if (this.$refs.ListOrTree && this.getChangeShow) {
            this.curTypeValue = 1;
            this.$refs.ListOrTree.changeCurType(1);
            this.$refs.menu.changeShowType(1);
          }
