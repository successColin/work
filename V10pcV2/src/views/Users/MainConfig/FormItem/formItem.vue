...
export default {
  ...
  computed: {
    getFeatureArr() {
      return this.configData.children[0];
    },
  },
  ...
  mounted() {
    ...
    // console.log(this.formData, this.getFeatureArr.form);
  },
  ...
  methods: {
    ...
    // 处理组件
    resoleComp() {
      const configArr = this.getFeatureArr.children;
      const arr = [];
      configArr.forEach((comp) => {
        if (comp.compName !== 'Label' && comp.canShow) {
          if (comp.labelNotChange) {
            if (comp.dataSource.columnName === 'orgid') {
              comp = {
                ...comp,
                component: 'editOrg',
              };
            } else if (comp.dataSource.columnName === 'roleIds') {
              comp = {
                ...comp,
                showTooltip: false,
                component: 'assRoles',
              };
            } else if (comp.dataSource.columnName === 'username') {
              comp = {
                ...comp,
                canReadonly: !!this.formData.id,
                component: comp.compName,
              };
            } else if (comp.dataSource.columnName === 'account') {
              comp = {
                ...comp,
                canReadonly: !!this.formData.id,
                component: comp.compName,
