    ...
    <div class="drawer__noScroll">
      ...
      <section class="module">
        ...
        <ul class="module__wrapper">
          <li
            class="module__item"
            v-for="(moduleObj, index) in getTraRouteArr"
            :key="moduleObj.id"
            @click="openMneuList(moduleObj)"
          >
            <div class="module__img">
...
export default {
  ...
  computed: {
    ...
    getFavArr() {
      return this.favItemArr.filter((menu) => menu.isCollect);
    },
    // 获取module
    getModule() {
    ...
    getFirstModule() {
      this.timer = setInterval(() => {
        ...
        if (this.getModule.length) {
          if (sessionStorage.moduleIndex === '-1') {
            bus.$emit('initMenuList', this.getTraRouteArr[0]);
          } else {
            bus.$emit(
              'initMenuList',
              this.getModule[sessionStorage.moduleIndex || 0],
            );
          }
        }
      }, 100);
