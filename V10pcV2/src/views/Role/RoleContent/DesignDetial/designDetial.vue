  ...
  <div class="design__listWrap">
    ...
    <div class="design__listwrap">
      ...
      <div class="listwrap__designListWrap" :key="update">
        <apiot-nodata
          description="暂无数据"
          v-if="!getAuthList.length"
        ></apiot-nodata>
        <div
          v-for="(item, index) in getAuthList"
          ...
          <div class="listwrap__itemBox">
            ...
            >
              <div
                ...
                :key="dataItem.value"
              >
                <div @click="(e) => changeRoleDataAuth(e, dataItem, item)">
                  <apiot-radio
                    class="raidoWrap"
                    :data-index="index"
                    :key="dataItem.value"
            ...
            >
              ...
              >
                <div
                  ...
                  :key="dataItem.value"
                >
                  <div @click="(e) => changeRoleDataAuth(e, dataItem, sub)">
                    <apiot-radio
                      class="raidoWrap"
...
export default {
  ...
  computed: {
    ...
    getSubCheckAll() {
      return function (obj) {
        ...
        const checkAll = functionAuthList.filter(
          (items) => items.check === '1',
        );
  ...
  methods: {
    async expand(item) {
      ...
      try {
        ...
        const sourceData = JSON.parse(
          JSON.stringify(this.authList[this.selectModuleIndex]),
        );
        const sourceList = sourceData.list;
        const i = sourceList.findIndex(
          (source) => source.menuId === item.menuId,
        );
        if (i !== -1) {
    ...
    updateInhertAuth(e, item) {
      ...
      const msg = `对${this.retrunName(
        this.designObj,
      )}${typeName}${doType}(${type}-${item.menuName})`;
      const params = {
        logData: msg,
          ...
          .forEach((item) => {
            item.addEventListener(
              ...
              (e) => {
                ...
                const currentIndex = path[2].getAttribute('data-index');
    ...
    changeRoleDataAuth(e, dataItem, functionObj) {
      ...
      const type =
        this.activeTabsKey === 'PCDesign'
          ...
          : this.$t('role.Mobileterminalpermission');
      const typeName = this.functionType === 'role' ? '角色' : '用户';
      const msg = `修改【${this.retrunName(
        this.designObj,
      )}】${typeName}的${this.showName(dataItem)}数据权限(${type}-${
    ...
    changeAllAuth(e, fun) {
      ...
      list.forEach((item) => {
        ...
        if (check !== checkValue) {
          newList.push({
            ...item,
            check: checkValue,
            [this.functionType === 'role' ? 'roleId' : 'userId']:
              this.designObj.id,
          });
        }
      });
      ...
      const typeName = this.functionType === 'role' ? '角色' : '用户';
      const authArr = newList.map((item) =>
        this.returnAuthName(item.permissionItemId),
      );
      const msg = `${doType}${this.retrunName(
        this.designObj,
      )}${typeName}的${authArr.join(',')}权限(${type}-${fun.menuName})`;
      this.doUpdateAuth(
        {
          list: newList,
          logData: msg,
    ...
    changeAuth(e, obj = {}, fun) {
      ...
      const msg = `${doType}${this.retrunName(
        this.designObj,
      )}${typeName}的${this.returnAuthName(obj.permissionItemId)}权限(${type}-${
        fun.menuName
      })`;
      this.doUpdateAuth(
        {
    ...
    async handleClickModules(functionObj) {
      ...
      this.$emit('selectChooseModules', functionObj);
