<template>
  <el-collapse v-model="activeNames" :accordion="true" class="offlineCheck">
    ...
    <el-collapse-item :title="$t('appconfig.deviceLayer')" name="2">
      ...
      <p class="offlineCheck--switch">
        {{ $t('appconfig.batchInspection') }}
        <el-switch
          class="switch"
          active-text="是"
          inactive-text="否"
          v-model="offlineCheck.deviceLayer.batchInspection"
        >
        </el-switch>
      </p>
    </el-collapse-item>
    <el-collapse-item :title="$t('appconfig.taskLayer')" name="3">
      ...
      <TaskColumn
        ...
        :columnArrBack.sync="offlineCheck.taskLayer.columnArr"
      ></TaskColumn>
    </el-collapse-item>
    <el-collapse-item :title="$t('appconfig.resultLayer')" name="4">
      <apiot-button
        class="offlineCheck--btn"
        @click="showResultColumnDialog = true"
      >
        <i class="iconfont icon-shezhi m-r-4"></i
        >{{ $t('appconfig.taskColumn') }}
      </apiot-button>
      <ResultColumn
        ...
        :columnArrBack.sync="offlineCheck.resultLayer.columnArr"
      ></ResultColumn>
    </el-collapse-item>
  </el-collapse>
</template>
