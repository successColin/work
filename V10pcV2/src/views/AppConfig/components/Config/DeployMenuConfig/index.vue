...
export default {
  ...
  computed: {
    ...
    showBg() {
      if (this.getGroup) {
        return (
          [2, 3].includes(this.getGroup.rowNum) && this.getGroup.showStyle === 1
        );
      }
      return false;
    },
  ...
  created() {
    ...
    if (this.activeObj.EnableCornerMarker === 1) {
  ...
  methods: {
    ...
    changeCornerMarker(val) {
      if (val === 1) {
        this.listSysDesignMenu();
        if (this.activeObj.menuType === 2) {
          this.activeObj.panelHornMarkId = this.activeObj.compId;
          this.activeObj.hornMarkNeedPermissions = true;
        }
      }
      if (val !== 1 && this.activeObj.panelHornMark) {
        this.activeObj.panelHornMark = [];
      }
    ...
    changeGroup() {
      ...
      const compIndex = this.configData.funcArea.groupArr[
        this.oldIndex
      ].menuArr.findIndex((item) => item.compId === this.activeObj.compId);
      this.configData.funcArea.groupArr[this.oldIndex].menuArr.splice(
        compIndex,
        1,
      );
      // 重新记录分组下标
      this.getOldIndex();
      // 把数据插入新的分组中
      this.configData.funcArea.groupArr[this.oldIndex].menuArr.push(
        this.activeObj,
      );
    },
    // 进入配置页面
    async enterConfig() {
      ...
      let { menuDesignId } = this.activeObj;
      if (!menuDesignId) {
        const data = await listSysDesignMenu({ menuId: this.activeObj.id });
        menuDesignId = data[0].id;
        this.activeObj.menuDesignId = menuDesignId;
      }
      const str = `/menuConfig/${this.activeObj.id}?menuName=${this.activeObj.menuName}&id=${menuDesignId}&isApp=1`;
