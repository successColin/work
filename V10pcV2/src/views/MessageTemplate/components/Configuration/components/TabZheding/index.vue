...
export default {
  ...
  methods: {
    init() {
      ...
      this.$refs.zzdingContent.init({ content: this.contentTemplate, variables: this.variables });
    },
    // 编辑聚焦时
  ...
  mounted() {
    ...
    // 向编辑器注入变量
    this.$bus.$on('zzdingContent_codemirror_markVal', (info) => {
      this.$refs.zzdingContent.setValue(info);
    });
  },

  beforeDestroy() {
    this.$bus.$off('zzdingContent_codemirror_markVal');
  }
};
</script>
