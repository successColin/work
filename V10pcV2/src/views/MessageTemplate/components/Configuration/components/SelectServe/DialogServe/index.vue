        ...
        <template slot="custom">
          ...
          >
            <template slot-scope="scope">
              <apiot-button
                v-if="scope.row.edit"
                @click.stop="save(scope.row, scope.row.index)"
                type="text"
                >{{ $t('common.save') }}</apiot-button
              >
              <apiot-button
                v-else
                @click.stop="handleEdit(scope.row, scope.row.index)"
                type="text"
                >{{ $t('common.edit1') }}</apiot-button
              >
              <apiot-button
                @click.stop="deleteServe(scope.row, scope.row.index)"
                type="text"
...
export default {
  components: { EditInputColumn, EditSelectColumn, EditUserColumn },
  ...
  computed: {
    ...
    logData() {
      ...
      this.columnData.forEach((item) => {
        ...
        if (item.dictName) {
          logItem.transFormArr = this.$store.getters.getCurDict(item.dictName);
        }
  ...
  methods: {
    ...
    async deleteServe(row, index) {
      ...
      try {
        ...
        this.$message.warning(serverNameError);
      }
    },
    // 校验整行
    verification(row) {
      ...
      for (let i = 0, len = columnData.length; i < len; i += 1) {
        ...
        // 寻找错误信息中是否有该错误
        const index = this.errorRow.findIndex(
