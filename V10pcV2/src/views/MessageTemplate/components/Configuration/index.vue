...
<template>
  <page-tabs class="configuration" :tabsArr="tabsArr" v-model="activitedTab">
    ...
    <tab-inner
      ...
      @change="changeInner"
    ></tab-inner>
    <tab-ding slot="com_5" @change="changeDing" :content="dingContent" />
    <tab-zheding
      slot="com_6"
      :content="zhedingContent"
      @change="changeZheding"
    ></tab-zheding>
  </page-tabs>
</template>
...
import TabZheding from './components/TabZheding';
...
export default {
  ...
  computed: {
    ...
    // 浙政钉配置内容
    zhedingContent() {
      return this.contents.find((item) => item.messageType === 6) || {};
    },
    tabsArr() {
      return (
  ...
  methods: {
    changeZheding(content) {
      this.changecOnfigurationObj(content, 6);
    },
    changeEmail(content) {
