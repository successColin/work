...
export default {
  data() {
    return {
      ...
      dropColumnData: [
        ...
        {
          label: 'importTemplate.enableDownUp',
          prop: 'downloadType',
  ...
  methods: {
    init() {
      this.$store.dispatch(
        'getCurrentDict',
        'REQUEST_TYPE,COLUMNS_TYPE,TEMPLATE_TYPE',
      );
    },
    // 按钮控制
    getButtonArr(arr) {
      ...
      if (!this.multiIdsString) {
        ...
        return this.$message({
          ...
          message: this.$t('placeholder.pleaseSelect', {
            any: this.$t('helpCenter.deleteData'),
          }),
        });
      }
      this.$confirm(
        this.$t('common.areYouSureToDelete', {
          name: this.$t('importTemplate.template'),
        }),
        this.$t('common.tip'),
        {
    ...
    async switchChange(row) {
      const data = await modifyDownloadType({
        id: row.id,
