...
export default {
  ...
  computed: {
    ...
    getTextStyles() {
      return function () {
        ...
        const { color1, color2, hShadow, vShadow, blur, shadowColor, ...rest } =
          stylesObj;
    ...
    getOption() {
      return function () {
        ...
        const str1 = labelValueType === 1 ? '{b}: {c}' : '{b}: {d}%';
        const str2 = labelValueType === 1 ? '{c}' : '{d}%';
        chartConfig = {
          ...chartConfig,
          ...
          label: {
            ...
            formatter: labelShowType === 'type' ? str1 : str2,
          },
          labelLine: {
                ...
                return new echarts.graphic.LinearGradient(1, 0, 0, 0, [
                  {
                    ...
                    offset: 0,
                    color:
                      colorList[params.dataIndex].c1 ||
                      colorList[params.dataIndex].c2 ||
                      '#fff',
                  },
                  {
                    offset: 1,
  ...
  methods: {
    ...
    initDom() {
      const { dataType, drillDownConfig = {} } = this.config;
      if (dataType === 3) {
      ...
      this.myChart.on('click', (params) => {
        const { interactionType, panelConfig, skipMenuConfig, dimension } =
          this.config;
        ...
        if (
          interactionType === 4 &&
          Array.isArray(drillDownField) &&
          drillDownField.length &&
          this.isFinished
        ) {
          // eslint-disable-next-line max-len
          this.doDrill(
    ...
    doDrill(
      ...
      if (
        tripStopField &&
        `${currentObj[tripStopField]}` === `${tripStopFieldValue}`
      ) {
        this.isFinished = true;
        return;
      }
      drillDownField.forEach((item) => {
        const { name: varName, value: filed } = item;
        ...
        if (menu) {
          ...
          menuConfig.menuFilter.forEach((item, index) => {
            ...
            if (filterTermType === 1) {
              ...
              menuConfig.menuFilter[index].filterTermStr =
                JSON.stringify(newFilterTermStr);
            }
            if (filterTermType === 2) {
              // sql过滤条件
              const str = this.reduceSqlFilter(filterTermSql);
              menuConfig.menuFilter[index].filterTermSql = Encrypt(str);
            }
          });
          const menuObj = {};
