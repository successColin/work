...
import { formatDate, IsURL, Decrypt, Encrypt, isJiami } from '@/utils/utils';
import {
  getRequestParams,
  validConditions,
...
export default {
  ...
  computed: {
    ...
    getColor() {
      return function (data, key) {
        ...
        if (conditionsArr.length) {
          const realConditionsArr = conditionsArr.filter(
            (item) => item.enableConditions,
          );
    ...
    getPrefixImg() {
      // 前缀图片
      const realImg = this.getColor(this.obj, 'fieldRelPrefixUrl');
      return this.$parseImgUrl(realImg || this.config.stylesObj.prefixImg);
    },
    getSuffixImg() {
      const realImg = this.getColor(this.obj, 'fieldRelSuffixUrl');
      return this.$parseImgUrl(realImg || this.config.stylesObj.suffixImg);
    },
    getPrefixImgStyle() {
      // 前缀图片样式
      const {
        stylesObj: { prefixImgMarginRight, prefixImgWidth, prefixImgHeight },
      } = this.config;
      return {
    ...
    getSuffixImgStyle() {
      // 后缀图片样式
      const {
        stylesObj: { suffixImgMarginLeft, suffixImgWidth, suffixImgHeight },
      } = this.config;
      return {
    ...
    getPrefixTextStyle() {
      ...
      const realColor = this.getColor(this.obj, 'fieldRelPrefixColor');
      return {
    ...
    renderContent() {
      ...
      if (dataType === 1) {
        return this.obj[takeEffect];
      }
      if (dataType === 3) {
        const { SQLEffect } = SqlDataConfig;
        return this.obj[SQLEffect];
      }
      return '';
    },
    getContentStyle() {
      // 内容具体样式
      const {
        stylesObj: { CFontFamily, CFontSize, CColor, CFontWeight },
      } = this.config;
      const realColor = this.getColor(this.obj, 'fieldRelColor');
      return {
    ...
    getSuffixTextStyle() {
      ...
      const realColor = this.getColor(this.obj, 'fieldRelSuffixColor');
      return {
  ...
  methods: {
    ...
    async reduceDataFilter(res) {
      ...
      const { SQLFilterFun, enableSQLFilter, SQLEffect, SQLDataFilterId } =
        SqlDataConfig;
