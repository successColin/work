...
border-radius: 4px 4px 0px 0px;margin:0 ${tabMarginLeftAndRight}px;border: 1px solid transparent;`;
        ...
        if (stylesType === 'styleThree') {
          if (this.active === `${obj.name}_${i}`) {
            styles = `padding:${activeConfig.cPaddingTopAndBottom}px ${activeConfig.cPaddingRightAndLeft}px;border-radius: ${activeConfig.borderRadius}px;
...
border-radius: ${unActiveConfig.borderRadius}px;margin:0 ${tabMarginLeftAndRight}px;
...
backgroundColor:${unActiveConfig.cBgUrl ? 'unset' : unActiveConfig.cBgColor};backgroundImage:url(${unActiveConfig.cBgUrl});backgroundSize: 100% 100%;`;
    ...
    getTabContentClass() {
      ...
      if (stylesType !== 'styleThree') {
        return 'border-bottom: 1px solid #E9E9E9;';
      }
      return '';
    },
    getRealStylesConfig() {
