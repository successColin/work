/** * @name: index * @author: DELL * @date: 2021/8/20 15:28 *
@description：index * @update: 2021/8/20 15:28 */
<!-- 页面 -->
<template>
...
const ApprovalMain = () =>
  import('./Components/ApprovalCom/component/ApprovalMain');
...
const TransverseBarChart = () =>
  import('./Components/TransverseBarChart/index');
...
const HorizontalProgressBar = () =>
  import('./Components/HorizontalProgressBar/index');
...
export default {
    ...
    getStylesBgUrl() {
      ...
      return function () {
        ...
        if (showType === 2 || showType === 4) {
          ...
          }');transform: scale(${this.transitionScaleX}, ${
  ...
  methods: {
    commonBus() {
      Bus.$off('previewOpera').$on(
        'previewOpera',
        ({ visible, imageFileName, imageFileUrl }) => {
      ...
      Bus.$off('lookApproval').$on('lookApproval', (config) => {
    ...
    async fetchElementData(list = []) {
      ...
      const targetList = list.filter((item) => {
        ...
        return (
          ['SingleLineTextHome', 'Dashboard'].includes(componentName) &&
          dataType === 3
        );
      });
      const paramArr = [];
    ...
    getParameters(config) {
      ...
      const { name } = this.$route;
      const query =
        name === 'appCustomPage' ? this.$route.query : this.otherParams;
      if (JSON.stringify(query) !== '{}') {
        Object.keys(this.otherParams).forEach((item) => {
          if (dataType === 3 && enableDataManage && variableConfig.length) {
            const currentVar = variableConfig.find(
              (varObj) => varObj.name === item,
            );
            if (currentVar) {
    ...
    setDrawContent() {
      const selector = this.otherParams.id
        ? `.files${this.otherParams.id}`
        : '.container__content';
      //  计算画布大小
      ...
      const widthScalc = (availWidth - 20) / (dWidth + 10 || 1920);
      const heightScalc = (availHeight - 10) / (dHeight || 1080);
      if (showType === 1) {
