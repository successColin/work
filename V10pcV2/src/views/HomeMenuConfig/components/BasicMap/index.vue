...
const { BmlHeatmap, BmInfoWindow, BmLabel, BaiduMap } = window.VueBaiduMap;
  ...
  computed: {
    ...
    getIcon() { //  渲染自定义图标
      return function (params) {
        const { stylesObj: {
          customDataMappingField, customDisplay = [], imageWidth, imageHeight
        } } = this.config;
    ...
    text() {
      ...
      if (showInfoType === 1 && customTipInfo) {
        return customTipInfo.replace(/(?<=\{)(.+?)(?=\})/g, (o) => this.pointInfo[o]).replaceAll('{', '').replaceAll('}', '');
      }
...
.singleTextWrap {
  ...
  ::v-deep{
    ...
    .BMap_pop>div:nth-child(1)>div, .BMap_pop>div:nth-child(3)>div,
