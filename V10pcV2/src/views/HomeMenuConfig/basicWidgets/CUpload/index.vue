  ...
  <div class="uploadGather">
    <div class="cUploadWrap" v-if="!isPublic">
      <div class="uploadImage" v-if="url">
        <el-image fit="contain" :src="$parseImgUrl(url)"></el-image>
        <div class="fileModal">
          <el-tooltip class="item" effect="dark" content="更换" placement="top">
            <span class="icon-qiehuan1 iconfont m-r-10" @click="click"></span>
          </el-tooltip>
      ...
      <div class="uploadWrap" v-if="!url" @click.stop.prevent="click">
        <el-upload
            v-on="$listeners"
            v-bind="$attrs"
    ...
    <MaterialLibrary ref="materialLibrary" @change="doChoose"/>
  </div>
</template>
...
export default {
  ...
  methods: {
    click() {
      this.$refs.materialLibrary.show();
    },
    error() {
    },
    handleRemove() {
