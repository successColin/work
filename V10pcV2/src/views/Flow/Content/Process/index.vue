...
import { editNode, getNodeInfo, updateNodeInfo, updateVersionTree } from '@/api/flow';
  ...
  methods: {
    ...
    async eventReciver({ event, args }) {
      ...
      if (['HISTORY', 'CURRENT'].includes(versionStatus)) {
        this.$message.warning('历史版本或者使用中的版本不允许添加删除节点');
        return;
      }
      ...
      try {
        ...
        if (type === 'condition') {
          newAttributes.tableName =
            this.data.type === 'start' ? this.data.properties.tableName : '';
        }
        if (!attributes) {
          this.nodeInfo = {};
        } else {
    ...
    download() {
      const dom = document.getElementById('cardMainHook');
      html2canvas(dom, {
        useCORS: true, // 允许跨域
        allowTaint: true,
        backgroundColor: '#f5f5f7', // 画布背景色，设置null为透明
        scale: 2, // 处理模糊问题
        dpi: 300, // 处理模糊问题
      }).then((canvas) => {
        ...
        a.download = this.activeFlow.workflowName; // 设置图片名称
        a.href = url; // 指定下载文件名称
      ...
      <div class="flow-wrap">
        ...
        <div class="flow-container" onClick={this.doCloseDrawer}>
          <div class="scale-slider">
            ...
            <el-tooltip className="item" effect="dark" content="下载流程图" placement="top-start">
              <i class="icon-xiazai iconfont" onClick={this.download}></i>
            </el-tooltip>
          </div>
          <flow-card
