...
<template>
  <div class="flowConfigWrap" @click="handleStop">
    ...
    <div class="config-content" v-if="!isVisible && currentVersion.id">
  ...
  methods: {
    ...
    handleOk(submitGlobal) {
      ...
      if (submitGlobal === 'globalConfig') {
        ...
        if (messageConfig.length) {
          ...
          if (obj) {
            ...
            this.$message.warning(`${typeObj[messageType]}的服务信息请配置完整!`);
            return;
          }
        }
        if (descConfigList.length) {
          let isTrue = false;
          descConfigList.forEach((item) => {
            if (!item.key || !item.name || !item.baseType) {
              isTrue = true;
            }
          });
          if (isTrue) {
      ...
      if (type === 'approver') {
        ...
        const {
          ...
          ccList,
          msgConf = {},
          checkAgreeFormConfigOrigin = [],
          appCheckAgreeFormConfigOrigin = [],
          rejectNodeConf
        } = obj;
        ...
        const { ROLE = [], ORG = [], USER = [], POST = [], NODE = [], FIELD = {} } = sourceType;
        if (
          !ROLE.length &&
          !ORG.length &&
          !USER.length &&
        ...
        const config = {
          ...
          checkAgreeFormConfig: checkAgreeFormConfigOrigin,
          appCheckAgreeFormConfig: appCheckAgreeFormConfigOrigin,
      ...
      if (type === 'condition') {
        ...
        termArr.forEach((arr = []) => {
          arr.forEach((item) => {
            const { columnObj = {}, content } = item;
            if (!columnObj.columnName || !content) {
      ...
      if (type === 'webhook') {
        ...
        // }
        if (!apiCode) {
          this.$message.warning('请填写代理接口编码！');
          return;
        }
    ...
    getConditionsMessage(termArr = [], termType) {
      ...
      const concatStr = (arr) => {
        let contentMessage = '';
        arr.forEach((item) => {
          const { lambda, valueType, content, columnObj } = item;
