        ...
        <el-table-column prop="key" label="字段">
          <template slot-scope="scope">
            <el-select
              ...
              filterable
              placeholder="请选择字段"
              @change="changeColumn($event, scope.row, scope.row.index, 'key')"
            >
              <el-option
                v-for="item in fieldOptions"
                :key="item.columnName"
        ...
        <el-table-column prop="enumeration" label="枚举">
          <template slot-scope="scope">
            <filterable-input
              ...
              :dialogType="3"
              @selectRes="
                changeColumn($event, scope.row, scope.row.index, 'enumeration')
              "
            ></filterable-input>
          </template>
        </el-table-column>
        <el-table-column prop="name" label="描述">
          <template slot-scope="scope">
            <apiot-input
              ...
              @input="changeColumn($event, scope.row, scope.row.index, 'name')"
            ></apiot-input>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleCancel() {
      ...
      // if (!this.checkType) return;
      if (this.checkType === 'pc') {
        this.checkFormConfigJSON = JSON.parse(
          JSON.stringify(this.checkFormConfigJSONOrigin),
        );
      } else {
        this.appCheckFormConfigJSON = JSON.parse(
          JSON.stringify(this.appCheckFormConfigJSONOrigin),
        );
      }
          ...
          arr.forEach((item) => {
            ...
            if (Object.prototype.hasOwnProperty.call(item, 'tableInfo')) {
              if (item.tableInfo.tableName) {
                const index = targetObj.findIndex(
                  (items) => items === item.tableInfo.tableName,
                );
