    ...
    <div class="add-node-btn-box flex  justify-center">
      <div class="add-node-btn">
        ...
        >
          ...
          <button class="btn add-children-node" type="button" slot="reference">
...
export default {
  ...
  computed: {
    getStyles() {
      return {
        transform: `scale(${this.scaleVal / 100})`,
        left: `${this.left}px`,
        top: `${this.top}px`
      };
    }
  ...
  methods: {
    ...
    getPosition(event) {
      ...
      const { clientX: startClientX, clientY: startClientY } = this.startPoint;
      this.left = Math.floor(clientX - startClientX) + this.originLeft;
      this.top = Math.floor(clientY - startClientY) + this.originTop;
    },
    dragEnd(e) {
      this.isDrag = false;
      this.getPosition(e);
  ...
  render(h) {
    return (
        <div class={ `cardMain${this.isDrag ? ' move' : ''}`} onMousedown={this.startDrag}>
          <div
              class="cardWrap"
              style={this.getStyles}
          >
            <div id="cardMainHook" style="display: inline-flex; flex-direction: column; align-items: center;transform-origin: 0 0;">
  ...
  beforeDestroy() {
    ...
    console.log('keydown---beforeDestroy');
    window.removeEventListener('mousemove', this.dragging);
    window.removeEventListener('mouseup', this.dragEnd);
  }
};
