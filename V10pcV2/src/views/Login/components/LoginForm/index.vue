...
<template>
  ...
  >
    ...
    <!-- 下载APP -->
    <sign-up :configs="configs" :isShowRegister="false"></sign-up>
  </el-form>
</template>
<script>
import { exchangeTokenLogin, postLoginForm } from '@/api/login.js';
        ...
        if (code === 'SS002') {
          ...
          switch (data.state) {
            case '1':
              try {
                await this.$confirm(
                  `${this.ruleForm.username}，您的账号密码已到有效期，请修改密码！`,
                  '账户安全提示',
                  {
                    dangerouslyUseHTMLString: true,
                    confirmButtonText: '确定',
            ...
            case '2':
              await this.$alert(
                `${this.ruleForm.username}，您的账号密码已到有效期，请联系管理员！`,
                '账户安全提示',
                {
                  confirmButtonText: '确定',
                },
            ...
            case '3':
              try {
                await this.$confirm(
                  `${this.ruleForm.username}，您的账号首次登录，请修改密码！`,
                  '账户安全提示',
                  {
                    dangerouslyUseHTMLString: true,
                    confirmButtonText: '确定',
