...
<template>
  <div class="contentWrap">
    ...
    ></PdfPreview>
    <SpecialFilePreview
      v-if="showSpecialFile"
      :visible.sync="showSpecialFile"
      :previewObj="previewObj"
    ></SpecialFilePreview>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      picList: [], // 文件列表
      pdfVisible: false,
      listTotal: 0, // 数据库所有文件的总和
      showSpecialFile: false,
    };
  },
  ...
  computed: {
    ...
    isShowCancel() {
      return function () {
        if (this.showType === 1) {
          if (this.groupType === 9) {
            if (this.selectKeys.length && this.pathArr.length === 1) {
              return true;
            }
            return false;
          }
      ...
      return function () {
        ...
        if (this.showType === 2) {
          ...
          if (this.selectKeys.length > 1) {
            ...
            if (this.selectKeys.length === arr.length) {
              return true;
            }
            return false;
          }
  ...
  methods: {
    // sortChange(sortObj) { // 排序
    //   this.specialInit({ orders: [sortObj] });
    // },
    loadMore() {
      console.log(
        '加载更多，',
        this.list.length,
        this.listTotal,
        this.list.length >= this.listTotal,
      );
    ...
    init(isNeedChange) {
      ...
      if (this.groupType === 'all') {
        // 全部
        const newPath = this.pathArr.at(-1);
        const params = {
          classId: this.showType,
          keywords: this.keyWord,
          parentId: newPath.id === 'all' ? 0 : newPath.id,
          isFolder: 0,
          current: this.current,
          size: this.size,
        };
      ...
      if (this.groupType === 7 || this.groupType === 8) {
        this.initKonwledge({}, isNeedChange);
        return;
      }
      if (this.groupType !== 'all') {
        // 分
    ...
    specialInit(params) {
      // 特殊
      const total = this.current * this.size;
      const n = this.list.length; //
      const nextCurrent = this.current + 1;
      if (n <= total && (total < this.listTotal || this.listTotal < total)) {
        this.initKonwledge(
          {
            size: total,
            current: 1,
      ...
      } else if (n < total && n === this.listTotal) {
        this.initKonwledge(
          {
            size: total,
            current: 1,
    ...
    async editFileName(name, fId) {
      ...
      try {
        ...
        if (this.groupType === 'all') {
          ...
          const changeType = [9, 10].includes(this.groupType) ? 'change' : '';
          await this.initKonwledge({}, changeType);
          this.selectKeys = [];
          this.edit = false;
        }
      } catch (e) {
    ...
    async handlePreview(item) {
      ...
      if (treeType === 1 && this.showType !== 3) {
        ...
        this.listTotal = 0;
        this.list = [];
        await this.initKonwledge(params, 'change');
        return false;
      }
      if (treeType === 1 && this.showType === 3) {
        // 文件夹进入下一个页面
      ...
      if (treeType === 2) {
        ...
        // } else
        if (this.$store.state.globalConfig.ureportConfig.filePreviewUrl) {
          this.previewObj = item;
          this.showSpecialFile = true;
        }
      }
      if (treeType === 3) {
        // 图片预览
    ...
    async initKonwledge(params, isNeedChange) {
      ...
      try {
        ...
        let data = {};
        if (this.groupType === 'all') {
          const pid = this.pathArr.at(-1).id;
          const param = {
            classId: this.showType,
            keywords: this.keyWord,
            parentId: pid === 'all' ? 0 : pid,
        ...
        const newTreeData = [9, 10, 7, 8].includes(this.groupType)
          ? treeData
          : treeData.records || [];
        this.loading = false;
        if (![9, 10, 7, 8].includes(this.groupType)) {
          this.listTotal = treeData.total ?? 0;
        }
        console.log(newTreeData, 'lieb', isNeedChange);
        if (this.groupType === 10) {
          ...
          // const lastList = newListData.sort((a, b) => {
    ...
    async handleSubmit() {
      ...
      try {
        ...
        if (this.groupType === 'all') {
          this.current = 1;
          await this.initKonwledge({}, 'change');
        }
        this.dialogVisible = false;
        this.selectKeys = [];
      } catch (e) {
    ...
    async handleAdd() {
      ...
      try {
        const currentFolder = this.pathArr.at(-1);
    ...
    getFileUrl(obj) {
      if (
        ...
        if (['vsdx', 'xmind', 'emmx', 'log', 'chm'].includes(suffix)) {
          return elseFile;
        }
        return xls;
      }
