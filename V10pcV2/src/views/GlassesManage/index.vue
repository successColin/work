...
<script>
import { getGlassesList, delGlasses } from '@/api/expert';
...
export default {
  data() {
    return {
      ...
      columnData: [
        ...
        // 类型
        {
          label: 'expert.spectacleModel',
          prop: 'modelnumber',
          compName: 'ElTableColumn'
        },
        // 类型
        {
          label: 'expert.serialNumber',
          prop: 'serialnumber',
          compName: 'ElTableColumn'
        },
        // 周期
        {
          label: 'expert.Presence',
          prop: 'stateid',
          compName: 'StateColumn'
        },
        // 类型
        {
          label: 'expert.onlineUsers',
          prop: 'onLineUserName',
          compName: 'UserColumn'
        },
        {
  ...
  methods: {
    async init() {
      ...
      this.targetUser = {};
      try {
        const res = await getGlassesList(this.params);
        const {
          total = 0,
          records = []
        } = res;
    ...
    deleteFun(row) {
      this.$confirm(`确定要删除${row.name || ''}眼镜吗？`,
          {
            type: 'warning',
            title: '提示'
          },
