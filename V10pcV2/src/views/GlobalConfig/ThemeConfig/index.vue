      ...
      <el-row>
        ...
        >
          <li>
            ...
            <div
              v-if="[0, 1, 2, 3, 4, 5, 6, 7, 8].includes(i)"
              style="flex: none"
              class="common"
            >
              <!-- <apiot-radio
              ...
              <ApiotColorPicker
                class="colorPicker"
                @change="changeRadio(approvalColor, item.approvalColor)"
                v-model="approvalColor"
                v-if="item.approvalColor && showColor"
              ></ApiotColorPicker>
              <el-checkbox
                  v-model="$store.state.globalConfig.themeConfig['enableBatchApproval']"
                  v-if="item.batchApproval"
                  class="m-l-10"
                  label="1"
                  true-label="1"
                  @change="(value) => changeRadio(value, 'enableBatchApproval')"
                  false-label="0">启用批量审批</el-checkbox>
            </div>
            <!-- 菜单样式 -->
            ...
            <div v-else-if="i === 13" class="common">
            ...
            <!-- 登录页LOGO -->
            <div v-else-if="i === 14" class="passwordConfig__logo common">
            ...
            <div v-else-if="i === 15" class="passwordConfig__logo common">
              <div v-if="!statement.isDefaultImage">
                <el-image
                  fit="cover"
                  style="width: 48px; height: 48px; border-radius: 4px"
                  :src="$parseImgUrl(homeImg)"
                  v-if="homeImg"
                  class="userImage m-r-4"
                >
                </el-image>
              ...
              <image-and-change
                v-if="statement.isDefaultImage"
                ref="defaultImage"
                :maxKb="20000"
                :imgObj="{
                  attributeValue:
                    $store.state.globalConfig.themeConfig.defaultImage,
                }"
                :showSlider="false"
              />
              <apiot-button
                type="text"
                class="passwordConfig__operation"
                @click="handleChangeCount('isDefaultImage')"
              >
                {{ changeBtnName(statement.isDefaultImage) }}
              </apiot-button>
            </div>
            <!-- socket地址 -->
...
export default {
  ...
  mounted() {
    ...
    this.approvalColor =
      this.$store.state.globalConfig.themeConfig.approvalColor;
  ...
  computed: {
    ...
    homeImg() {
      const url = this.$store.state.globalConfig.themeConfig.defaultImage;
      // eslint-disable-next-line max-len
      return url && url.substr(0, url.length - 1);
    },
    getValue() {
    ...
    configArr() {
      return [
        ...
        {
          name: this.$t('globalConfig.enableGlobalSearch'),
          col: 12,
          attr: 'enableGlobalSearch',
        },
        {
        ...
        {
          name: this.$t('globalConfig.defaultImage'),
          col: 24,
          attr: 'defaultImage',
        },
        // {
  ...
  methods: {
    ...
    initImgObj() {
      Object.keys(this.imgObj).forEach((key) => {
        this.imgObj[key] =
          this.$store.state.globalConfig.themeConfigArr.find(
    ...
    async changeWidth(width) {
      ...
      if (!fileList.length) {
        this.$store.commit('setThemeConfig', {
          key: 'homePageLogo',
          value: '',
        });
      }
      this.$store.commit('setThemeConfig', {
        key: 'homePageLogoWidth',
        value: width,
      });
    },
    async update(params) {
    ...
    async handleChangeCount(key) {
      ...
      if (key === 'isRegistration' && this.statement[key]) {
        // 登录logo保存
        const arr = this.$store.state.globalConfig.themeConfigArr || [];
        const currentWidthObj = arr.find(
          (item) => item.attributeKey === 'homePageLogoWidth',
        );
      ...
      if (key === 'isDefaultImage') {
        // 首页图片
        await this.uploadSuccess('defaultImage');
      }
      if (key === 'isTopHeight') {
      ...
      if (key === 'istopStyle') {
        this.changeRadio(
          this.$store.state.globalConfig.themeConfig.topStyle,
          'topStyle',
        );
        this.uploadSuccess('topStyleIcon');
      }
      if (key === 'isSocketUrl') {
        await this.changeRadio(
          this.$store.state.globalConfig.themeConfig.socketUrl,
          'socketUrl',
        );
      }
      if (key === 'isUreportUrl') {
        this.changeRadio(
          this.$store.state.globalConfig.ureportConfig.ureportUrl,
          'ureportUrl',
        );
      }
      this.statement[key] = !this.statement[key];
