      ...
      <el-row>
        ...
        >
          <li v-show="!(i === 8 && loginStyle === 1)">
            ...
            <!-- 登录页轮播图 -->
            <div v-if="i === 13" class="passwordConfig__logo common">
...
export default {
  ...
  computed: {
    ...
    loginImg() {
      ...
      return (
        ...
        this.loginObj.attributeValue.substr(
          0,
          this.loginObj.attributeValue.length - 1,
        )
      );
    },
    backgroundImg() {
    ...
    configArr() {
      ...
      return [
        ...
        {
          name: [
            this.$t('globalConfig.passwordError'),
            this.$t('globalConfig.passwordErrorShow'),
          ],
          col: 12,
        ...
        // 11
        {
          name: this.$t('globalConfig.enableDownQr'),
          col: colStyleVal,
          key: 'enableDownQr',
          attributeKey: 'enableDownQr',
        },
        // 12
        {
    ...
    showValueName() {
      return function (options, val) {
        const obj =
          options && options.find((item) => item.value === Number(val));
        return obj && obj.name;
      };
    },
  },
  methods: {
    ...
    handleRemove({ url }) {
      this.$confirm(
        this.$t('common.areYouSureToDelete', {
          name: this.$t('helpCenter.image'),
        }),
    ...
    async init() {
      ...
      await this.$store.dispatch(
        'getCurrentDict',
        'SCAN_TYPE,SSO_TYPE_PC,LOGIN_BOOT_MODE',
      );
      ...
      const errorObj =
        this.response.find(
          (item) => item.attributeKey === 'sliderErrorsCount',
        ) || {};
      const loginModepcObj =
        this.response.find((item) => item.attributeKey === 'loginModePc') || {};
      const recordObj =
        this.response.find((item) => item.attributeKey === 'record') || {};
      const welcomeTitleObj =
        this.response.find((item) => item.attributeKey === 'welcomeTitle') ||
        {};
      const sysObj =
        this.response.find((item) => item.attributeKey === 'systemTitle') || {};
      const enableRegistrationObj =
        this.response.find(
          (item) => item.attributeKey === 'enableRegistration',
        ) || {};
      const scanType =
        this.response.find((item) => item.attributeKey === 'scanType') || {};
      const enableForgetPasswordObj =
        this.response.find(
          (item) => item.attributeKey === 'enableForgetPassword',
        ) || {};
      const enableMultilingual =
        this.response.find(
          (item) => item.attributeKey === 'enableMultilingual',
        ) || {};
      const styleObj =
        this.response.find((item) => item.attributeKey === 'style') || {};
      const ssoTypePcObj =
        this.response.find((item) => item.attributeKey === 'ssoTypePc') || {};
      const themeColorObj =
        this.response.find((item) => item.attributeKey === 'themeColor') || {};
      const stylePercentage =
        this.response.find((item) => item.attributeKey === 'stylePercentage') ||
        {};
      const logoObj =
        this.response.find((item) => item.attributeKey === 'loginLogo') || {};
      const backgroundObj =
        this.response.find((item) => item.attributeKey === 'backgroundImage') ||
        {};
      const enableLoginFirstPc =
        this.response.find(
          (item) => item.attributeKey === 'enableLoginFirstPc',
        ) || {};
      const enableDownQr =
        this.response.find((item) => item.attributeKey === 'enableDownQr') ||
        {};

      const enableAPPDownload =
        this.response.find(
          (item) => item.attributeKey === 'enableAPPDownload',
        ) || {};
      const logoObjWidth =
        this.response.find((item) => item.attributeKey === 'loginLogoWidth') ||
        {};
      const zzdConfig =
        this.response.find((item) => item.attributeKey === 'zzdConfig') || {};
      const loopObj =
        this.response.find((item) => item.attributeKey === 'loginLoopMaps') ||
        {};
      const loopMapsArr = loopObj.attributeValue
        ? loopObj.attributeValue.split(',')
        : [];
      this.loopMaps = [];
      ...
      this.params = {
        ...
        enableDownQr: enableDownQr.attributeValue,
      ...
      this.zzdConfig =
        (zzdConfig.attributeValue && JSON.parse(zzdConfig.attributeValue)) ||
        {};
    ...
    async saveLoopMap() {
      const currentObj = this.response.find(
        (item) => item.attributeKey === 'loginLoopMaps',
      );
      const formData = new FormData();
      ...
      if (this.statement[key]) {
        ...
        if (key === 'isLoginStyle') {
          ...
          params = [
            ...
            {
              ...this.response.find(
                (item) => item.attributeKey === 'stylePercentage',
              ),
        ...
        if (key === 'isRegistration') {
          ...
          const widthObj = this.response.find(
            (item) => item.attributeKey === 'loginLogoWidth',
          );
          params = {
            ...widthObj,
