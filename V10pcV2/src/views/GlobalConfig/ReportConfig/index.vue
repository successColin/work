        ...
        >
          <li>
            ...
            <div v-if="i === 3" class="common">
              <div v-if="!statement.isFilePreviewUrl" style="line-height: 48px">
                {{ config.filePreviewUrl }}
              </div>
              <apiot-input
                style="width: 90%"
                v-else
                :isForbid="false"
                v-model="config.filePreviewUrl"
              ></apiot-input>
              <apiot-button
                type="text"
                class="homeConfig__operation"
                @click="handleChangeCount('isFilePreviewUrl')"
              >
                {{ changeBtnName(statement.isFilePreviewUrl) }}
              </apiot-button>
            </div>
            <div v-if="i === 4" class="common">
              <div
                v-if="!statement.isFilePreviewOrigin"
                style="line-height: 48px"
              >
                {{ config.filePreviewOrigin }}
              </div>
              <apiot-input
                style="width: 90%"
                v-else
                :isForbid="false"
                v-model="config.filePreviewOrigin"
              ></apiot-input>
              <apiot-button
                type="text"
                class="homeConfig__operation"
                @click="handleChangeCount('isFilePreviewOrigin')"
              >
                {{ changeBtnName(statement.isFilePreviewOrigin) }}
              </apiot-button>
            </div>
          </li>
        </el-col>
...
export default {
  data() {
    return {
      ...
      statement: {
        ...
        isAppUrl: false,
        isShareUrl: false,
        isFilePreviewUrl: false,
        isFilePreviewOrigin: false,
      },
    };
  },
  computed: {
    ...
    leftConfigArr() {
      return [
        ...
        {
          name: this.$t('globalConfig.filePreviewUrl'),
          col: 24,
          attr: 'ureportUrl',
        },
        {
          name: this.$t('globalConfig.filePreviewOrigin'),
          col: 24,
          attr: 'ureportUrl',
        },
      ];
  ...
  methods: {
    ...
    async handleChangeCount(key) {
      ...
      if (key === 'isFilePreviewUrl' && this.statement[key]) {
        this.changeRadio(this.config.filePreviewUrl, 'filePreviewUrl');
      }
      if (key === 'isFilePreviewOrigin' && this.statement[key]) {
        this.changeRadio(this.config.filePreviewOrigin, 'filePreviewOrigin');
      }
      this.statement[key] = !this.statement[key];
    },
  },
