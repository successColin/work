  ...
  <section class="passwordConfig">
    <ul>
      <li v-for="(item, i) in configArr" :key="`${item.name}_${i}`">
        ...
        <!-- 密码强度 -->
        <div v-else-if="i === 2">
          <div v-if="!isClickPasswordStrength">
            {{ showValueName(passwordRule, passwordStrengthVal) }}
          </div>
          <apiot-select
            v-else
            v-model="passwordStrengthVal"
          ...
          <apiot-button
            ...
            @click="handlePasswordStrength"
          >
            {{ changeBtnName(isClickPasswordStrength) }}
          </apiot-button>
        </div>
        <!-- 密码禁止内容 -->
        <div v-else-if="i === 3">
          ...
          <apiot-button
            type="text"
            class="passwordConfig__operation"
            @click="handleSetProhibitContent"
          >
            设置
          </apiot-button>
        </div>
    ...
    <prohibit-content
      title="设置密码禁止内容"
      :value.sync="prohibitContentVal"
      :visible.sync="dialogProhibitContent"
      @handleProhibit="handleProhibit"
    ></prohibit-content>
  </section>
</template>

<script>
import { getListByKey, updateSysGlobalParameters, commonUpdate } from '@/api/globalConfig';
import { Decrypt, passwordRule } from '@/utils/utils';
import ChangePassword from '../components/ChangePassword';
import HistoryPassword from '../components/HistoryPassword';
import ProhibitContent from '../components/ProhibitContent';
...
export default {
  ...
  data() {
    return {
      ...
      // 密码强度
      dialogProhibitContent: false,
      // 是否点击有效期
      isClickPeriodValidity: false,
      // 是否点击
      isClickOnlineTime: false,
      // 是否点击密码强度
      isClickPasswordStrength: false,
      // 下拉框__密码规则
      passwordStrengthVal: '',
      // 下拉框__有效期值
      periodValidityValue: '',
  ...
  computed: {
    ...
    configArr() {
      return [
        ...
        {
          name: this.$t('globalConfig.cryptographicStrength')
        },
        {
          name: this.$t('globalConfig.passwordForbiddenContent')
        },
        {
  ...
  methods: {
    // 设置禁止内容按钮
    handleSetProhibitContent() {
      this.dialogProhibitContent = true;
    },
    ...
    async getListData() {
      ...
      this.loginConfig = (await getListByKey({ parameterKey: 'LOGIN' })) || [];
      if (this.loginConfig.length) {
        // 密码强度
        const passwordStrengthValObj = this.loginConfig.find(
          (item) => item.attributeKey === 'passwordStrength'
        );
        this.passwordStrengthVal = Number(passwordStrengthValObj.attributeValue) || 1;
        // 禁止内容
        const prohibitContentValObj = this.loginConfig.find(
          (item) => item.attributeKey === 'prohibitContent'
        );
        this.prohibitContentVal = prohibitContentValObj.attributeValue || '';
        console.log(this.prohibitContentVal);
      }
    ...
    // },
    // 按钮__密码强度
    async handlePasswordStrength() {
      this.isClickPasswordStrength = !this.isClickPasswordStrength;
      if (!this.isClickPasswordStrength) {
        const passwordStrengthValObj = this.loginConfig.find(
          (item) => item.attributeKey === 'passwordStrength'
        );
        const params = {
          ...passwordStrengthValObj,
          attributeValue: this.passwordStrengthVal
        };
    ...
    async handleProhibit() {
      ...
      const params = {
        ...prohibitContentValObj,
        attributeValue: this.prohibitContentVal
      };
      await commonUpdate({ list: [params] });
      this.$message({
        type: 'success',
    ...
    handleChangeSelectVal(name, val) {
      ...
      } else if (name === 'passwordStrengthVal') {
        this.passwordStrengthVal = val;
      }
    }
  }
};
