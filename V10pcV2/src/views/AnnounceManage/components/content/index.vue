...
import {
  ...
  getAnnounceList,
  modifyAnnounce,
  switchAnnounceLocation
} from '@/api/announceManage.js';
  ...
  methods: {
    ...
    async getListData() {
      ...
      this.tableData = res.records.map((item, index) => {
        ...
        const roleNames = item.roleNames ? item.roleNames.split(',') : [];
        const orgIds = item.orgIds ? item.orgIds.split(',') : [];
        const orgNames = item.orgNames ? item.orgNames.split(',') : [];
        const postIds = item.postIds ? item.postIds.split(',') : [];
        const postNames = item.postNames ? item.postNames.split(',') : [];
        ...
        for (let i = 0; i < postIds.length; i += 1) {
          postusers.push({
            username: postNames[i],
            id: postIds[i],
            type: 4,
          });
        }
        obj.childIds = {
          1: obj.childIds ? obj.childIds.split(',') : [],
          2: obj.roleIds ? obj.roleIds.split(',') : [],
          3: obj.orgIds ? obj.orgIds.split(',') : [],
          4: obj.postIds ? obj.postIds.split(',') : [],
        };
        obj.leaders = leaders;
        obj.roleusers = roleusers;
        obj.orgusers = orgusers;
        obj.postusers = postusers;
        obj.groups = [
          ...roleusers,
          ...orgusers,
          ...postusers,
