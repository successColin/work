...
<template>
  <apiot-sidebar
    ...
    @doAddGroup="insertAnnounceGroup"
...
import {
  ...
  listAnnounceGroup,
  modifyAnnounceGroup,
  switchAnnounceGroupLocation
} from '@/api/announceManage';
...
export default {
  ...
  methods: {
    ...
    async initGroup() {
      ...
      try {
        const res = await listAnnounceGroup();
        this.groupList = res;
    ...
    async doUpdate(name) {
      ...
      try {
        ...
        await modifyAnnounceGroup(params);
        this.$emit('update:asideLoading', false);
        this.initGroup(true);
      } catch (e) {
        const Obj = {
    ...
    selectList(item) {
      // 切换分组
      console.log();
      this.$bus.$emit('announceGroup', item);
    },
    doAfterHide() {
      // 输入框值清空值
      this.input = '';
    ...
    async insertAnnounceGroup() {
      ...
      try {
        ...
        const { sno: sno1 } = obj;
        const data = await insertAnnounceGroup({
          sno: sno1 + 1,
