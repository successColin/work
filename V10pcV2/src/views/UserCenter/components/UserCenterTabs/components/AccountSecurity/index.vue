...
export default {
  ...
  data() {
      ...
      if (value === '') {
        ...
        if (length < 8) {
          ...
          callback(
            new Error(
              this.$t(
                'login.helloPasswordsMustBeLongerThan6DigitsAndLessThan18Digits',
              ),
            ),
          );
        } else if (value === this.curData.oldPassword) {
          // 现在密码与旧密码对比
          callback(
            new Error(
              this.$t(
                'login.theOriginalPasswordAndTheNewPasswordCannotBeTheSame',
              ),
            ),
          );
        } else {
          ...
          if (this.passwordStrengthVal === 0) {
            ...
            if (securityCheckTwo.test(value)) {
              ...
              callback(
                this.$t(
                  'common.anyThreeTypesOfNumbersLettersAndPunctuationMarks',
                ),
              );
            }
    ...
    const confirmPassword = (rule, value, callback) => {
      if (value === '') {
        callback(
          new Error(this.$t('login.helloTherePleaseEnterThePasswordAgain')),
        );
      } else if (value !== this.curData.password) {
        callback(
          new Error(
            this.$t('login.HelloTheTwoPasswordsDoNotMatchPleaseReEnterThem'),
          ),
        );
      } else if (value === this.curData.oldPassword) {
        callback(
    ...
    return {
      ...
      rules: {
        ...
        password: [
          { validator: password, trigger: ['blur', 'change'], required: true },
        ],
        confirmPassword: [
          {
