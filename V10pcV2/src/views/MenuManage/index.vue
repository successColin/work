  ...
  >
    ...
    >
      ...
      >
        ...
        <el-form-item :label="$t('menu.openType')" prop="openType">
          <el-radio v-model="form.openType" :label="1">{{
            $t('menu.inner')
          }}</el-radio>
          <el-radio v-model="form.openType" :label="2">{{
            $t('menu.outer')
          }}</el-radio>
        </el-form-item>
        <el-form-item
...
export default {
  ...
  methods: {
    ...
    // 添加菜单
    addMenu() {
      const icon = `icon-caidan${Math.floor(Math.random() * 39) + 1}`;
    ...
    dragMove(evt) {
      ...
      this.relateGroupId = -1;
      if (
        evt.relatedContext.element.type === 'menu' &&
        evt.relatedContext.element.state === 2
      ) {
        return false;
      }
    },
