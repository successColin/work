    ...
    activeFlow: {
      ...
      handler(val) {
        if (JSON.stringify(val) !== '{}') {
          ...
          this.list = [];
          this.$bus.$emit('chooseFlowVersion', {});
        }
      }
    }
  },
  ...
  methods: {
    ...
    async fetchVersionList(flag = true) {
      ...
      try {
        ...
        // 首先找到有没有在使用中的流程
        const useingVersion = this.list.find((item) => item.versionStatus === 'CURRENT');
        if (useingVersion && flag) {
          this.$bus.$emit('chooseFlowVersion', useingVersion);
        } else if (flag) {
