import { addNode, delVersionNodeTree } from '@/api/flow';
import nodeConfig from './config.js';

// eslint-disable-next-line no-unused-vars
const isEmpty = (data) => data === null || data === undefined || data === '';
const isEmptyArray = (data) => (Array.isArray(data) ? data.length === 0 : true);

export class NodeUtils {
  constructor() {
    this.state = {
      flowVersionId: null, // 节点id
      notAllow: ['HISTORY', 'CURRENT'], // 不允许操作的状态
    };
  }

  /**
   * 根据自增数生成64进制id
   * @returns 64进制id字符串
   */
  static idGenerator() {
    let qutient = new Date() - new Date('2020/08/01');
    qutient += Math.ceil(Math.random() * 1000); // 防止重複
    const chars =
      '0123456789ABCDEFGHIGKLMNOPQRSTUVWXYZabcdefghigklmnopqrstuvwxyz';
    const charArr = chars.split('');
    const radix = chars.length;
    const res = [];
    do {
      const mod = qutient % radix;
      qutient = (qutient - mod) / radix;
      res.push(charArr[mod]);
    } while (qutient);
    return res.join('');
  }

  /**
   * 判断节点类型
   * @param {Node} node - 节点数据
   * @returns Boolean
   */
  static isConditionNode(node) {
    return node && node.type === 'condition';
  }

  static isCopyNode(node) {
    return node && node.type === 'copy';
  }

  static isStartNode(node) {
    return node && node.type === 'start';
  }

  static isApproverNode(node) {
    return node && node.type === 'approver';
  }

  static isFillInNode(node) {
    return node && node.type === 'fill';
  }

  static isBackEndNode(node) {
    return node && node.type === 'backend_data';
  }

  static isWebHookNode(node) {
    return node && node.type === 'webhook';
  }

  static isProcedureNode(node) {
    return node && node.type === 'procedure';
  }

  static isNoticeNode(node) {
    return node && node.type === 'notice';
  }

  static setFlowVersinId(id) {
    this.state = {
      ...this.state,
      flowVersionId: id,
    };
  }

  /**
   *
   * @param obj 检查的对象
   * @param key 检查对象中的字段
   * @returns {boolean}
   */
  static isExistInObj(obj, key) {
    return Object.prototype.hasOwnProperty.call(obj, key);
  }

  /**
   * 判断该值是否是对象
   * @param obj
   * @returns {boolean}
   */
  static isObject(obj) {
    return Object.prototype.toString.call(obj) === '[object Object]';
  }

  /**
   * 创建指定节点
   * @param { String } type - 节点类型
   * @param { Object } previousNodeId - 父节点id
   * @returns { Object } 节点数据
   */
  static async createNode(type, previousNodeId) {
    const { flowVersionId } = this.state;
    const res = JSON.parse(JSON.stringify(nodeConfig[type]));
    const data = await addNode({
      nodeType: type.toUpperCase(),
      prevNodeId: previousNodeId,
      workflowVersionId: flowVersionId,
      nodeContent: res.content,
      nodeTitle: res.properties.title,
    });
    return new Promise((resolve) => {
      res.nodeId = data.id;
      res.prevId = previousNodeId;
      resolve(res);
    });
    // eslint-disable-next-line no-unreachable
    // return res;
  }

  /**
   * 获取指定节点的父节点（前一个节点）
   * @param { String } prevId - 父节点id
   * @param { Object } processData - 流程图全部数据
   * @returns { Object } 父节点
   */
  static getPreviousNode(prevId, processData) {
    if (processData.nodeId === prevId) return processData;
    if (processData.childNode) {
      const r1 = this.getPreviousNode(prevId, processData.childNode);
      if (r1) {
        return r1;
      }
    }
    if (processData.conditionNodes) {
      // eslint-disable-next-line no-restricted-syntax
      for (const c of processData.conditionNodes) {
        const r2 = this.getPreviousNode(prevId, c);
        if (r2) {
          return r2;
        }
      }
    }
  }

  /**
   * 递归获取所有被删除节点的id
   * @param deletedNode  - 被删除节点的数据
   * @param deletedArr - 被删除节点的id集合
   * @returns {*[]}
   */
  static getAllDeletedNodes(deletedNode = {}, deletedArr = [], processData) {
    const prevNode =
      this.getPreviousNode(deletedNode.prevId, processData) || {};
    if (prevNode.type === 'empty') {
      const { conditionNodes } = prevNode;
      if (
        conditionNodes &&
        Array.isArray(conditionNodes) &&
        conditionNodes.length > 2
      ) {
        if (
          this.isExistInObj(deletedNode, 'childNode') &&
          this.isObject(deletedNode.childNode)
        ) {
          this.getAllDeletedNodes(
            deletedNode.childNode,
            deletedArr,
            processData,
          );
        }
        if (
          this.isExistInObj(deletedNode, 'conditionNodes') &&
          Array.isArray(deletedNode.conditionNodes) &&
          deletedNode.conditionNodes.length
        ) {
          deletedNode.conditionNodes.forEach((item) => {
            this.getAllDeletedNodes(item, deletedArr, processData);
          });
        }
        if (!deletedArr.includes(deletedNode.nodeId)) {
          deletedArr.push(deletedNode.nodeId);
        }
      } else {
        if (
          conditionNodes &&
          Array.isArray(conditionNodes) &&
          conditionNodes.length === 2
        ) {
          conditionNodes.forEach((item) => {
            if (
              this.isExistInObj(item, 'childNode') &&
              this.isObject(item.childNode)
            ) {
              this.getAllDeletedNodes(item.childNode, deletedArr, processData);
            }
            if (
              this.isExistInObj(item, 'conditionNodes') &&
              Array.isArray(item.conditionNodes) &&
              item.conditionNodes.length
            ) {
              deletedNode.conditionNodes.forEach((items) => {
                this.getAllDeletedNodes(items, deletedArr, processData);
              });
            }
            if (!deletedArr.includes(item.nodeId)) {
              deletedArr.push(item.nodeId);
            }
          });
        }
        if (!deletedArr.includes(prevNode.nodeId)) {
          deletedArr.push(prevNode.nodeId);
        }
      }
    } else {
      if (deletedNode.nodeId) {
        if (!deletedArr.includes(deletedNode.nodeId)) {
          deletedArr.push(deletedNode.nodeId);
        }
      }
      // if (this.isExistInObj(deletedNode, 'childNode') && this.isObject(deletedNode.childNode)) {
      //   this.getAllDeletedNodes(deletedNode.childNode, deletedArr, processData);
      // }
      if (
        this.isExistInObj(deletedNode, 'conditionNodes') &&
        Array.isArray(deletedNode.conditionNodes) &&
        deletedNode.conditionNodes.length
      ) {
        deletedNode.conditionNodes.forEach((item) => {
          this.getAllDeletedNodes(item, deletedArr, processData);
        });
      }
      if (
        this.isExistInObj(deletedNode, 'childNode') &&
        this.isObject(deletedNode.childNode)
      ) {
        this.getAllDeletedNodes(deletedNode.childNode, deletedArr, processData);
      }
    }
    return deletedArr;
  }

  static getDelNodes(nodeData, delArr) {
    delArr.push(nodeData.nodeId);
    if (
      this.isExistInObj(nodeData, 'childNode') &&
      this.isObject(nodeData.childNode)
    ) {
      this.getDelNodes(nodeData.childNode, delArr);
    }
    if (
      this.isExistInObj(nodeData, 'conditionNodes') &&
      Array.isArray(nodeData.conditionNodes) &&
      nodeData.conditionNodes.length
    ) {
      nodeData.conditionNodes.forEach((item) => {
        this.getDelNodes(item, delArr);
      });
    }
    return delArr;
  }

  /**
   * 删除节点
   * @param { Object  } nodeData - 被删除节点的数据
   * @param { Object  } processData - 流程图的所有节点数据
   */
  static async deleteNode(nodeData, processData, checkEmpty = true) {
    const delNode = (nodeIds) => {
      delVersionNodeTree({ nodeId: nodeIds });
    };
    // eslint-disable-next-line no-shadow
    const concatChild = (prev, delNode) => {
      prev.childNode = delNode.childNode;
      if (delNode.type !== 'empty') {
        // eslint-disable-next-line no-unused-expressions
        isEmptyArray(prev.conditionNodes) &&
          (prev.conditionNodes = delNode.conditionNodes);
      }
      // eslint-disable-next-line no-unused-expressions
      prev.childNode && (prev.childNode.prevId = prev.nodeId);
      // eslint-disable-next-line no-return-assign,no-unused-expressions
      prev.conditionNodes &&
        // eslint-disable-next-line no-return-assign
        prev.conditionNodes.forEach((c) => (c.prevId = prev.nodeId));
    };
    const prevNode = this.getPreviousNode(nodeData.prevId, processData) || {};
    if (nodeData.type !== 'condition') {
      // 删除的不是分支
      concatChild(prevNode, nodeData);
      delNode(nodeData.nodeId);
      return;
    }

    if (this.isConditionNode(nodeData)) {
      // 删除的是分支
      const cons = prevNode.conditionNodes || [];
      const index = cons.findIndex((c) => c.nodeId === nodeData.nodeId);
      if (index === -1) return;
      if (cons.length > 2) {
        // 如果删除的分支大于2，就直接删除该分支
        // const delArr = this.getDelNodes(nodeData, []);
        try {
          // await delNode(delArr.join(','));
          await delNode(nodeData.nodeId);
          cons.splice(index, 1);
        } catch (e) {
          // console.log(e);
        }
      } else {
        const prevEmptyNode = prevNode; // 获取empty节点
        // 获取empty的父节点
        const parentNode =
          this.getPreviousNode(prevEmptyNode.prevId, processData) || {};
        if (prevEmptyNode.type === 'empty') {
          const anotherCon = cons[+!index];
          const { childNode: endNode } = prevEmptyNode; // 结束节点
          // 如果分支节点有子节点，需要把分支节点的数据拼接到父节点去
          if (anotherCon.childNode) {
            const { childNode } = endNode;
            // 如果分支结束节点有子节点，需要把分支结束的子节点拼接到分支节点的最后子节点上
            if (childNode) {
              let lastNode = anotherCon;
              while (lastNode.childNode) {
                lastNode = lastNode.childNode;
              }
              lastNode.childNode = childNode;
              lastNode.childNode.prevId = lastNode.nodeId;
            }
            concatChild(parentNode, anotherCon);
          } else {
            const { childNode } = endNode;
            // 如果分支结束节点有子节点，需要把分支结束的子节点拼接到分支节点的最后子节点上
            if (childNode) {
              childNode.prevId = parentNode.nodeId;
              parentNode.childNode = childNode;
            } else {
              delete parentNode.childNode;
            }
          }
        }
        await delNode(nodeData.nodeId);
      }
      return;
    }
    if (checkEmpty && prevNode.type === 'empty') {
      if (this.isConditionNode(nodeData)) {
        const willDelBranch = prevNode.conditionNodes.length === 2;
        const target = willDelBranch ? prevNode : nodeData;
        this.deleteNode(target, processData, willDelBranch);
      } else {
        if (isEmptyArray(prevNode.conditionNodes)) {
          this.deleteNode(prevNode, processData);
        }
        this.deleteNode(nodeData, processData, false);
      }
      // this.deleteNode( prevNode, processData )
      // !this.isConditionNode(nodeData) && this.deleteNode(nodeData, processData)
      return;
    }
    const deletedArr = this.getAllDeletedNodes(nodeData, [], processData);
    // const delNode = (nodeIds) => {
    //   delVersionNodeTree({ nodeIds });
    // };
    if (this.isConditionNode(nodeData)) {
      const cons = prevNode.conditionNodes || [];
      const index = cons.findIndex((c) => c.nodeId === nodeData.nodeId);
      if (index === -1) return;
      if (cons.length > 2) {
        deletedArr.push(cons[index].nodeId);
        const newArr = Array.from(new Set(deletedArr));
        delNode(newArr.join(','));
        cons.splice(index, 1);
      } else {
        const anotherCon = cons[+!index];
        prevNode.conditionNodes.forEach((item) => {
          deletedArr.push(item.nodeId);
        });
        const newArr = Array.from(new Set(deletedArr));
        delNode(newArr.join(','));
        delete prevNode.conditionNodes;
        if (prevNode.childNode) {
          let endNode = anotherCon;
          while (endNode.childNode) {
            endNode = endNode.childNode;
          }
          endNode.childNode = prevNode.childNode;
          endNode.childNode.prevId = endNode.nodeId;
        }
        concatChild(prevNode, anotherCon);
        if (prevNode.childNode && prevNode.childNode.type === 'empty') {
          this.deleteNode(prevNode.childNode, prevNode);
        }
      }
      // 重新编排优先级
      // eslint-disable-next-line no-return-assign
      cons.forEach((c, i) => (c.properties.priority = i));
      return;
    }
    concatChild(prevNode, nodeData);
    delNode(deletedArr.join(','));
  }

  // TODO:
  // static copyNode ( nodeData, processData ) {
  //   let prevNode = this.getPreviousNode( nodeData.prevId, processData )
  //   let index = prevNode.conditionNodes.findIndex( c => c.nodeId === nodeData.nodeId )

  // }
  /**
   * 添加审计节点（普通节点 approver）
   * @param { Object } data - 目标节点数据，在该数据节点之后添加审计节点
   * @param { Object } isBranchAction - 目标节点数据，是否是条件分支
   * @param { Object } newChildNode - 传入的新的节点 用户操作均为空  删除操作/添加抄送人 会传入该参数 以模拟添加节点
   */
  static async addApprovalNode(data, isBranchAction, newChildNode = undefined) {
    const oldChildNode = data.childNode;
    if (!newChildNode) {
      newChildNode = await this.createNode('approver', data.nodeId);
    }
    data.childNode = newChildNode;
    if (oldChildNode) {
      newChildNode.childNode = oldChildNode;
      oldChildNode.prevId = newChildNode.nodeId;
    }
    const { conditionNodes } = data;
    if (
      Array.isArray(conditionNodes) &&
      !isBranchAction &&
      conditionNodes.length
    ) {
      newChildNode.conditionNodes = conditionNodes.map((c) => {
        // console.log(newChildNode);
        c.prevId = newChildNode.nodeId;
        return c;
      });
      delete data.conditionNodes;
    }
    if (
      oldChildNode &&
      oldChildNode.type === 'empty' &&
      newChildNode.type !== 'empty' &&
      oldChildNode.conditionNodes.length === 0
    ) {
      this.deleteNode(oldChildNode, data);
    }
  }

  /**
   * 添加空节点
   * @param { Object } data - 空节点的父级节点
   * @return { Object } emptyNode - 空节点数据
   */
  static async addEmptyNode(data) {
    const emptyNode = await this.createNode('empty', data.nodeId);
    const emptyEndNode = await this.createNode(
      'condition_group_end',
      emptyNode.nodeId,
    );
    await this.addApprovalNode(data, true, emptyNode);
    await this.addApprovalNode(emptyNode, true, emptyEndNode);
    return emptyNode;
  }

  static addCopyNode(data, isBranchAction) {
    // console.log(data, 'addCopyNode', isBranchAction);
    // 复用addApprovalNode  因为抄送人和审批人基本一致
    this.addApprovalNode(
      data,
      isBranchAction,
      this.createNode('copy', data.nodeId),
    );
  }

  /**
   * 添加填写节点
   * @param data
   * @param isBranchAction
   */
  static async addFillInNode(data, isBranchAction) {
    let newChildNode = await this.createNode('fill', data.nodeId);
    const oldChildNode = data.childNode;
    if (!newChildNode) {
      newChildNode = await this.createNode('fill', data.nodeId);
    }
    // newChildNode = newChildNode || await this.createNode('approver', data.nodeId);
    // eslint-disable-next-line no-unused-expressions
    data.childNode = newChildNode;
    if (oldChildNode) {
      newChildNode.childNode = oldChildNode;
      oldChildNode.prevId = newChildNode.nodeId;
    }
    const { conditionNodes } = data;
    if (
      Array.isArray(conditionNodes) &&
      !isBranchAction &&
      conditionNodes.length
    ) {
      newChildNode.conditionNodes = conditionNodes.map((c) => {
        // console.log(newChildNode);
        c.prevId = newChildNode.nodeId;
        return c;
      });
      delete data.conditionNodes;
    }
    if (
      oldChildNode &&
      oldChildNode.type === 'empty' &&
      newChildNode.type !== 'empty' &&
      oldChildNode.conditionNodes.length === 0
    ) {
      this.deleteNode(oldChildNode, data);
    }
  }

  /**
   * 添加后台数据节点
   * @param data
   * @param isBranchAction
   * @returns {Promise<void>}
   */
  static async addBackEndDataNode(data, isBranchAction) {
    let newChildNode = await this.createNode('backend_data', data.nodeId);
    const oldChildNode = data.childNode;
    if (!newChildNode) {
      newChildNode = await this.createNode('backend_data', data.nodeId);
    }
    data.childNode = newChildNode;
    if (oldChildNode) {
      newChildNode.childNode = oldChildNode;
      oldChildNode.prevId = newChildNode.nodeId;
    }
    const { conditionNodes } = data;
    if (
      Array.isArray(conditionNodes) &&
      !isBranchAction &&
      conditionNodes.length
    ) {
      newChildNode.conditionNodes = conditionNodes.map((c) => {
        // console.log(newChildNode);
        c.prevId = newChildNode.nodeId;
        return c;
      });
      delete data.conditionNodes;
    }
    if (
      oldChildNode &&
      oldChildNode.type === 'empty' &&
      newChildNode.type !== 'empty' &&
      !oldChildNode.conditionNodes.length
    ) {
      this.deleteNode(oldChildNode, data);
    }
  }

  /**
   * 添加存储过程节点
   * @param data
   * @param isBranchAction
   * @returns {Promise<void>}
   */
  static async addProcedureNode(data, isBranchAction) {
    let newChildNode = await this.createNode('procedure', data.nodeId);
    const oldChildNode = data.childNode;
    if (!newChildNode) {
      newChildNode = await this.createNode('procedure', data.nodeId);
    }
    data.childNode = newChildNode;
    if (oldChildNode) {
      newChildNode.childNode = oldChildNode;
      oldChildNode.prevId = newChildNode.nodeId;
    }
    const { conditionNodes } = data;
    if (
      Array.isArray(conditionNodes) &&
      !isBranchAction &&
      conditionNodes.length
    ) {
      newChildNode.conditionNodes = conditionNodes.map((c) => {
        // console.log(newChildNode);
        c.prevId = newChildNode.nodeId;
        return c;
      });
      delete data.conditionNodes;
    }
    if (
      oldChildNode &&
      oldChildNode.type === 'empty' &&
      newChildNode.type !== 'empty' &&
      !oldChildNode.conditionNodes.length
    ) {
      this.deleteNode(oldChildNode, data);
    }
  }

  /**
   * 添加外部接口节点
   * @param data
   * @param isBranchAction
   * @returns {Promise<void>}
   */
  static async addWebHookNode(data, isBranchAction) {
    let newChildNode = await this.createNode('webhook', data.nodeId);
    const oldChildNode = data.childNode;
    if (!newChildNode) {
      newChildNode = await this.createNode('webhook', data.nodeId);
    }
    data.childNode = newChildNode;
    if (oldChildNode) {
      newChildNode.childNode = oldChildNode;
      oldChildNode.prevId = newChildNode.nodeId;
    }
    const { conditionNodes } = data;
    if (
      Array.isArray(conditionNodes) &&
      !isBranchAction &&
      conditionNodes.length
    ) {
      newChildNode.conditionNodes = conditionNodes.map((c) => {
        // console.log(newChildNode);
        c.prevId = newChildNode.nodeId;
        return c;
      });
      delete data.conditionNodes;
    }
    if (
      oldChildNode &&
      oldChildNode.type === 'empty' &&
      newChildNode.type !== 'empty' &&
      !oldChildNode.conditionNodes.length
    ) {
      this.deleteNode(oldChildNode, data);
    }
  }

  /**
   * 添加消息通知节点
   * @param data
   * @param isBranchAction
   * @returns {Promise<void>}
   */
  static async addMessageNode(data, isBranchAction) {
    let newChildNode = await this.createNode('notice', data.nodeId);
    const oldChildNode = data.childNode;
    if (!newChildNode) {
      newChildNode = await this.createNode('notice', data.nodeId);
    }
    data.childNode = newChildNode;
    if (oldChildNode) {
      newChildNode.childNode = oldChildNode;
      oldChildNode.prevId = newChildNode.nodeId;
    }
    const { conditionNodes } = data;
    if (
      Array.isArray(conditionNodes) &&
      !isBranchAction &&
      conditionNodes.length
    ) {
      newChildNode.conditionNodes = conditionNodes.map((c) => {
        // console.log(newChildNode);
        c.prevId = newChildNode.nodeId;
        return c;
      });
      delete data.conditionNodes;
    }
    if (
      oldChildNode &&
      oldChildNode.type === 'empty' &&
      newChildNode.type !== 'empty' &&
      !oldChildNode.conditionNodes.length
    ) {
      this.deleteNode(oldChildNode, data);
    }
  }

  /**
   * 添加条件节点 condition 通过点击添加条件进入该操作
   * @param { Object } data - 目标节点所在分支数据，在该分支最后添加条件节点
   */
  static async appendConditionNode(data) {
    const conditions = data.conditionNodes;
    const node = await this.createNode('condition', data.nodeId);
    // eslint-disable-next-line no-shadow
    const defaultNodeIndex = conditions.findIndex(
      // eslint-disable-next-line no-shadow
      (node) => node.properties.isDefault,
    );
    // node.properties.priority = conditions.length;
    if (defaultNodeIndex > -1) {
      conditions.splice(-1, 0, node); // 插在倒数第二个
      // 更新优先级
      // node.properties.priority = conditions.length - 2;
      // conditions[conditions.length - 1].properties.priority = conditions.length - 1;
    } else {
      conditions.push(node);
    }
    this.setDefaultCondition(node, data);
  }

  /**
   * 添加条件分支 branch
   * @param { Object } data - 目标节点所在节点数据，在该节点最后添加分支节点
   */
  // eslint-disable-next-line no-unused-vars
  static async appendBranch(data, isBottomBtnOfBranch) {
    // isBottomBtnOfBranch 用户点击的是分支树下面的按钮
    const nodeData = data;
    // // 由于conditionNodes是数组 不能添加下级分支 故在两个分支树之间添加一个不会显示的正常节点 兼容此种情况
    // if (Array.isArray(data.conditionNodes) && data.conditionNodes.length) {
    //   if (isBottomBtnOfBranch) {
    //     // 添加一个模拟用的空白节点并返回这个节点，作为新分支的父节点
    //     nodeData = await this.addEmptyNode(nodeData, true);
    //   } else {
    //     const emptyNode = await this.addEmptyNode(nodeData, true);
    //     emptyNode.conditionNodes = nodeData.conditionNodes;
    //     emptyNode.conditionNodes.forEach((n) => {
    //       n.prevId = emptyNode.nodeId;
    //     });
    //   }
    // }
    // const res = await this.createNode('condition', nodeData.nodeId);
    // const res1 = await this.createNode('condition', nodeData.nodeId);
    // const conditionNodes = [res, res1].map((c, i) => {
    //   c.properties.title += i + 1;
    //   c.properties.priority = i;
    //   return c;
    // });
    // // eslint-disable-next-line no-unused-expressions
    // nodeData.conditionNodes = conditionNodes;
    // // console.log(conditionNodes, 'conditionNodes');
    const emptyNode = await this.addEmptyNode(nodeData, true);
    const res = await this.createNode('condition', emptyNode.nodeId);
    const res1 = await this.createNode('condition', emptyNode.nodeId);
    const conditionNodes = [res, res1].map((c, i) => {
      c.properties.title += i + 1;
      c.properties.priority = i;
      return c;
    });
    emptyNode.conditionNodes = conditionNodes;
    emptyNode.conditionNodes.forEach((n) => {
      n.prevId = emptyNode.nodeId;
    });
  }

  /**
   * 重设节点优先级（条件节点）
   * @param {Node} cnode - 当前节点
   * @param {Number} oldPriority - 替换前的优先级（在数组中的顺序）
   * @param {Node} processData - 整个流程图节点数据
   */
  static resortPrioByCNode(cnode, oldPriority, processData) {
    // 当前节点为默认节点 取消修改优先级
    if (cnode.properties.isDefault) {
      cnode.properties.priority = oldPriority;
      return;
    }
    const prevNode = this.getPreviousNode(cnode.prevId, processData);
    const newPriority = cnode.properties.priority;
    // 替换节点为默认节点 取消修改优先级
    if (prevNode.conditionNodes[newPriority].properties.isDefault) {
      cnode.properties.priority = oldPriority;
      return;
    }
    const delNode = prevNode.conditionNodes.splice(newPriority, 1, cnode)[0];
    delNode.properties.priority = oldPriority;
    prevNode.conditionNodes[oldPriority] = delNode;
  }

  /**
   * 提升条件节点优先级——排序在前
   * @param { Object } data - 目标节点数据
   * @param { Object  } processData - 流程图的所有节点数据
   */
  static increasePriority(data, processData) {
    if (data.properties.isDefault) {
      // 默认节点不能修改优先级
      return;
    }
    // 分支节点数据 包含该分支所有的条件节点
    const prevNode = this.getPreviousNode(data.prevId, processData);
    const branchData = prevNode.conditionNodes;
    const index = branchData.findIndex((c) => c === data);
    if (index) {
      // 和前一个数组项交换位置 Array.prototype.splice会返回包含被删除的项的集合（数组）
      branchData[index - 1].properties.priority = index;
      branchData[index].properties.priority = index - 1;
      // eslint-disable-next-line prefer-destructuring
      branchData[index - 1] = branchData.splice(
        index,
        1,
        branchData[index - 1],
      )[0];
    }
  }

  /**
   * 降低条件节点优先级——排序在后
   * @param { Object } data - 目标节点数据
   * @param { Object  } processData - 流程图的所有节点数据
   */
  static decreasePriority(data, processData) {
    // 分支节点数据 包含该分支所有的条件节点
    const prevNode = this.getPreviousNode(data.prevId, processData);
    if (!prevNode) return;
    const branchData = prevNode.conditionNodes;
    const index = branchData.findIndex((c) => c.nodeId === data.nodeId);
    if (index < branchData.length - 1) {
      const lastNode = branchData[index + 1];
      if (lastNode.properties.isDefault) {
        // 默认节点不能修改优先级
        return;
      }
      // 和后一个数组项交换位置 Array.prototype.splice会返回包含被删除的项的集合（数组）
      lastNode.properties.priority = index;
      branchData[index].properties.priority = index + 1;
      // eslint-disable-next-line prefer-destructuring
      branchData[index + 1] = branchData.splice(
        index,
        1,
        branchData[index + 1],
      )[0];
    }
  }

  /**
   * 当有其他条件节点设置条件后 检查并设置最后一个节点为默认节点
   * @param {Node} cnode  - 当前节点
   * @param {Node} processData - 整个流程图节点数据或父级节点数据
   */
  static setDefaultCondition(cnode, processData) {
    const DEFAULT_TEXT = '其他情况进入此流程';
    const conditions = this.getPreviousNode(
      cnode.prevId,
      processData,
    ).conditionNodes;
    // eslint-disable-next-line max-len
    const hasCondition = (node) =>
      node.properties &&
      (node.properties.initiator || !isEmptyArray(node.properties.conditions));
    const clearDefault = (node) => {
      node.properties.isDefault = false;
      // eslint-disable-next-line no-unused-expressions
      node.content === DEFAULT_TEXT && (node.content = '请设置条件');
    };
    const setDefault = (node) => {
      node.properties.isDefault = true;
      node.content = DEFAULT_TEXT;
    };
    let count = 0;
    conditions.slice(0, -1).forEach((node) => {
      // eslint-disable-next-line no-plusplus,no-unused-expressions
      hasCondition(node) && count++;
      clearDefault(node);
    });
    const lastNode = conditions[conditions.length - 1];
    // eslint-disable-next-line no-unused-expressions
    count > 0 && !hasCondition(lastNode)
      ? setDefault(lastNode)
      : clearDefault(lastNode);
  }

  /**
   * 校验单个节点必填项完整性
   * @param {Node} node - 节点数据
   */
  // eslint-disable-next-line no-unused-vars
  static checkNode(node, parent) {
    // 抄送人应该可以默认自选
    let valid = true;
    const props = node.properties;
    // eslint-disable-next-line no-unused-expressions
    this.isStartNode(node) && !props.initiator && (valid = false);
    // eslint-disable-next-line no-unused-expressions
    this.isConditionNode(node) &&
      !props.isDefault &&
      !props.initiator &&
      isEmptyArray(props.conditions) &&
      (valid = false);

    const customSettings = ['myself', 'optional', 'director'];
    // eslint-disable-next-line no-unused-expressions
    this.isApproverNode(node) &&
      !customSettings.includes(props.assigneeType) &&
      isEmptyArray(props.approvers) &&
      (valid = false);
    return valid;
  }

  /**
   * 判断所有节点是否信息完整
   * @param {Node} processData - 整个流程图数据
   * @returns {Boolean}
   */
  static checkAllNode(processData) {
    let valid = true;
    const loop = (node, callback, parent) => {
      // eslint-disable-next-line no-unused-expressions
      !this.checkNode(node, parent) && callback();
      if (node.childNode) loop(node.childNode, callback, parent);
      if (!isEmptyArray(node.conditionNodes)) {
        node.conditionNodes.forEach((n) => loop(n, callback, node));
      }
    };
    // eslint-disable-next-line no-return-assign
    loop(processData, () => (valid = false));
    return valid;
  }
}

/**
 * 添模拟数据
 */
export function getMockData() {
  const startNode = NodeUtils.createNode('start');
  startNode.childNode = NodeUtils.createNode('approver', startNode.nodeId);
  return startNode;
}
