      ...
      >
        ...
        >
          ...
          <el-select
            ...
            @change="lambdaChange(child)"
          >
            <el-option
              :label="lambda.label"
              :value="lambda.value"
              v-for="lambda in getLanmbdaArr"
              :key="lambda.value"
            ></el-option>
          </el-select>
          <el-select
            ...
            :disabled="['is null', 'is not null'].includes(child.lambda)"
          >
...
export default {
  ...
  computed: {
    // 获取lanmbda数据
    getLanmbdaArr() {
      if (this.noNullLambda) {
        return this.lambdaArr.slice(0, -2);
      }
      return this.lambdaArr;
    },
    getRealTableName() {
  ...
  methods: {
    // lambdaChange lambda 公式更改
    lambdaChange(child) {
      if (['is null', 'is not null'].includes(child.lambda)) {
        child.valueType = 1;
        child.content = '';
      }
    },
