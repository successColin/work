<template>
  ...
  >
        ...
        <draggable v-model="leftBtnArr" group="btnArr" animation="300">
          ...
          >
            ...
            <i
              ...
              v-if="!['FilterSelectCol'].includes(item.compName)"
    ...
    <div class="btnArea__pBox m-b-10" v-if="$route.query.isApp === '1'">
      <p class="btnArea__p">样式风格</p>
      <el-select
        class="btnArea__select"
        placeholder="请选择类型"
        v-model="getBtnsArr.appRadius"
      >
        <el-option label="椭圆" :value="1"></el-option>
        <el-option label="圆角" :value="2"></el-option>
        <el-option label="直角" :value="3"></el-option>
      </el-select>
    </div>
    <div class="btnArea__pBox m-b-10" v-if="$route.query.isApp === '1'">
  ...
  methods: {
    ...
    initLeftAndRightArr() {
      if (this.isBtnArea) {
        ...
        this.leftBtnArr = this.getBtnsArr.children.slice(
          0,
