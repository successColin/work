      ...
      <div class="rule__body">
        <div class="rule__scroll">
          ...
          >
            <div
              class="rule__item"
              v-for="(item, index) in activeObj.bgColorArr"
              :key="item.id"
            >
              ...
              <span class="rule__item--tip">
                <apiot-color-picker class="colorSelect" v-model="item.color">
                </apiot-color-picker>
              </span>
...
export default {
  ...
  methods: {
    addRule() {
      if (!this.activeObj.bgColorArr) {
        this.$set(this.activeObj, 'bgColorArr', []);
      }
      this.activeObj.bgColorArr.push({
        content: '',
        color: '#fff',
    ...
    async deleteRule(index) {
      try {
        ...
        this.activeObj.bgColorArr.splice(index, 1);
      } catch (error) {
        // console.log(error);
      }
    },
  ...
  watch: {
    showRuleDialog(v) {
      console.log(v);
      if (!v) {
        const { length } = this.activeObj.bgColorArr;
        for (let i = length - 1; i >= 0; i -= 1) {
          if (!this.activeObj.bgColorArr[i].content) {
            this.activeObj.bgColorArr.splice(i, 1);
          }
        }
      }
    },
