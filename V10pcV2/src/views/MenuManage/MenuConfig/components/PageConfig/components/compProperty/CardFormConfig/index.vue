    ...
    <section class="contentConfig__wrapper" v-if="!isBtnArea">
      ...
      <div class="contentConfig__box" v-if="!isSelect">
        ...
        <el-select
          ...
          class="contentConfig__select"
        >
          <el-option label="背景展示" :value="1"></el-option>
          <el-option label="表格展示" :value="2"></el-option>
        </el-select>
      </div>
      <div
        class="contentConfig__box"
        v-if="!isSelect && getCurrentTab.cardType === 1"
      >
        <h2>圆角展示</h2>
        <el-input-number
          v-model.number="getCurrentTab.iconColumn.radius"
          controls-position="right"
          :min="0"
          :max="5"
          class="contentConfig__box--radius"
        ></el-input-number>
        <apiot-color-picker
          initColor="#ffffff"
          class="m-l-8"
          v-model="activeObj.iconColumn.radiusBg"
        >
        </apiot-color-picker>
      </div>
      <div class="contentConfig__box" v-if="!isSelect">
        <h2>Lable名称</h2>
        <el-select
          v-model="activeObj.iconColumn.style"
          style="width: 220px"
          class="m-l-10"
        >
        ...
        <apiot-color-picker v-model="activeObj.iconColumn.color" class="m-l-8">
        </apiot-color-picker>
      </div>
      <div
        class="contentConfig__box"
      ...
      >
        ...
        <el-input-number
          ...
          class="m-l-10"
          v-model.number="getCurrentTab.iconColumn.lineDis"
          controls-position="right"
          :min="0"
          :max="12"
        ></el-input-number>
      ...
      if (this.activeObj.iconColumn.columnName) {
        ...
        const arr =
          this.activeObj.pageType === 2
            ...
            : this.activeObj.children[0].children;
        const obj = arr.find(
          (item) => item.compId === this.activeObj.iconColumn.compId,
        );
        if (obj) {
