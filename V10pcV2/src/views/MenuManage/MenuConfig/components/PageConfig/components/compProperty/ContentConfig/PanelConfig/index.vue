<template>
  <section>
    <apiot-dialog
      ...
      cancelBtnName="确认"
    >
      <el-form label-position="top" label-width="80px" :key="key">
        <div class="form--line">
          <el-form-item
            label="关联面板"
            class="form--child"
...
export default {
  ...
  data() {
    return {
      key: 0,
      curPaneObj: JSON.parse(JSON.stringify(initPaneObj)),
      // 弹窗类型数组
      dialogNameArr: [
  ...
  created() {
    if (
      ...
      this.getPaneConfig();
    } else if (this.isCustomPage && JSON.stringify(this.tabPaneConfig) !== '{}') {
      this.curPaneObj = this.tabPaneConfig;
      this.getPaneConfig();
    } else {
      this.curPaneObj = JSON.parse(JSON.stringify(initPaneObj));
    }
    this.$nextTick(() => {
      this.key += 1;
    });
    ...
    async selectPane(pane) {
      ...
      if (!pane) {
        ...
        if (!this.isCustomPage) {
          ...
          this.$set(this.configData[0].paneObj, this.getPanelCompId, this.curPaneObj);
        }
        return;
      }
