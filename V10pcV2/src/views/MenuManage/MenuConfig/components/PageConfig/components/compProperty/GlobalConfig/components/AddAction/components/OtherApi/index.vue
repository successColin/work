    ...
    <section class="action__api">
      <el-form label-position="top" label-width="80px" @submit.native.prevent>
        ...
        <el-form-item
          ...
          class="form--child"
          key="GET"
          v-if="actionTriggerObj.requestMethod === 'GET'"
        >
          ...
          <GetParameter
            v-if="showJson"
            :formulaStr.sync="actionTriggerObj.requestParameter"
            :triggerCompMap="triggerCompMap"
            :showContent="actionTriggerObj.requestMethod === 'GET'"
            ref="getParameter"
          ></GetParameter>
        </el-form-item>
        <el-form-item
...
export default {
  ...
  methods: {
    methodChange() {
      ...
      if (this.actionTriggerObj.requestMethod === 'POST') {
        this.actionTriggerObj.requestParameter = '{}';
      } else {
        this.actionTriggerObj.requestParameter = '';
      }
    ...
    getSelectComp(comp) {
      ...
      if (this.$refs.getParameter) {
        this.$refs.getParameter.nodeClick(comp);
      }
    },
    setReturnValue() {
