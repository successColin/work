            ...
            >
              <h1 class="ToMenuConfig__li--head">
                ...
                <el-tooltip
                  ...
                  content="过滤条件是否应用于子查询"
                  placement="bottom"
                >
                  <el-switch
                    v-if="filter.showChildSearch === 1"
                    v-model="filter.enableChildSearch"
                    class="switchBox__switch m-t-4 m-l-4"
                    :active-value="1"
                    :inactive-value="2"
                  >
                  </el-switch>
      ...
      return (item) => {
        ...
        keys.forEach((key) => {
          ...
          if (area.compName === 'DevicePosTree') {
            ...
            if (area.compName === 'TreeMain') {
              area.showChildSearch = 1;
            } else {
              area.showChildSearch = 2;
            }
            if (index === -1) {
              obj[`${key}-${area.tableInfo.tableName}`] = area;
            }
          } else {
            const index = item.menuFilter.findIndex((panle) => {
              if (panle.compId === key) {
  ...
  methods: {
    ...
    async getPaneConfig(menu) {
      if (menu.id && !this.menuConfig[menu.id]) {
        ...
        this.$set(this.menuConfig, menu.id, data[0].designOverallLayout);
      }
    },
    // 新增菜单
    async addMenu(v) {
    ...
    handleCommand({ area, item }) {
      ...
      item.menuFilter.push({
        ...
        filterTermSql: '',
        termParams: '',
        showChildSearch: area.showChildSearch,
        enableChildSearch: 1,
      });
    },
    // 删除区域
