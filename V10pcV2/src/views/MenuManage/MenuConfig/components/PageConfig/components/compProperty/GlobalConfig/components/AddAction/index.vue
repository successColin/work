<template>
  ...
  >
    <el-form label-position="top" label-width="80px" class="form" ref="action">
      ...
      <div
        class="form--line"
        :style="`${triggerEvent === 2 ? '' : 'width: calc(50% - 15px)'}`"
      >
        ...
        <el-form-item
          ...
          v-if="triggerEvent === 2"
        >
          <el-select v-model="containStart">
  ...
  computed: {
    ...
    getTriggerEvent() {
      if (this.getCompType === 16) {
        return [
          this.$store.state.dictManage.TRIGGER_EVENT.find(
            (item) => item.value === 1,
          ),
        ];
      }
      if (this.getCompType === 5) {
        return this.$store.state.dictManage.TRIGGER_EVENT.filter(
          (item) => item.value !== 2,
        );
      }
      return this.$store.state.dictManage.TRIGGER_EVENT.filter(
    ...
    getCompType() {
      if (
        ...
        this.triggerCompMap[this.triggerComp.compId]
      ) {
  ...
  watch: {
    getTriggerEvent(v) {
      const index = v.findIndex(
        (trigger) => trigger.value === this.triggerEvent,
      );
      if (index === -1) {
  ...
  methods: {
    ...
    sureClick() {
      ...
      const index = this.actionList.findIndex((action) => {
        ...
        action.triggerContainStart = this.containStart;
        action.sysMenuDesignId = this.isPanel
          ? this.sysMenuDesignId
          : this.$route.query.id;
        // 条件里面组件 公式先不管
        if (action.effectiveConditions) {
          const term = JSON.parse(
            action.effectiveConditions.replace(/\\/g, ''),
          );
          // console.log(term);
    ...
    async getDesignTriggers() {
      ...
      this.containStart = data[0].triggerContainStart || 1;
      // data.forEach((item) => {
      //   if (!item.termObj) {
      //     item.termObj = {
