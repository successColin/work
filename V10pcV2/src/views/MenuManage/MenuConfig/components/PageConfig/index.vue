  ...
  <div class="pageConfig">
    <header class="pageConfig__header">
      ...
      <div class="pageConfig__header--right">
        <div
          class="pageConfig__header--proto"
          :class="[{ active: enabledPrototype }]"
        >
          <span v-if="!enabledPrototype">关闭原型模式</span>
          <span v-else>开启原型模式</span>
          <el-switch
            class="switch"
            v-model="enabledPrototype"
            :active-value="true"
            :inactive-value="false"
            @change="enabledPrototypeChange"
          >
          </el-switch>
        </div>
        <apiot-button @click="previewPage">预览</apiot-button>
    ...
    <section class="pageConfig__wrapper">
      ...
      <div class="pageConfig__wrapper--center">
        ...
        <div
          ...
          @click.stop="
            $route.query.isApp != 1 && !isSelect && !isUser
    ...
    <transition name="slide-bottom">
      <PreviewPage
        :panelName="isPanel ? panelName : $route.query.menuName"
      ...
      <PreviewPageApp
        :panelName="isPanel ? panelName : $route.query.menuName"
        :visible.sync="showPreviewApp"
        :menuId="$route.params.id"
        :panelId="panelRow ? panelRow.id : ''"
        v-else-if="showPreviewApp"
      ></PreviewPageApp>
    </transition>
  </div>
</template>
...
import { getDesignPage, modifyDesignPage } from '@/api/user.js';
import { Decrypt, Encrypt, createUnique } from '@/utils/utils';
...
import CardFormConfig from './components/compProperty/CardFormConfig';
...
import InductionNFCConfig from './components/compProperty/InductionNFCConfig';
import InductionScanConfig from './components/compProperty/InductionScanConfig';
...
import FlowHisListConfig from './components/compProperty/FlowHisListConfig';
...
import SignatureConfig from './components/compProperty/SignatureConfig';
...
import SwitchConfig from './components/compProperty/SwitchConfig';
import TableMainConfig from './components/compProperty/TableMainConfig';
import GanttMainConfig from './components/compProperty/GanttMainConfig';
...
import UserGlobalConfig from './components/compProperty/UserGlobalConfig';
import WebviewConfig from './components/compProperty/WebviewConfig';
import PreviewPage from './components/previewPage';
import PreviewPageApp from './components/previewPageApp';
...
export default {
            ...
            bigArea.children.forEach((tab) => {
              ...
              if (tab.children.length) {
                tab.children.forEach((area) => {
                  if (area.textPanelId) {
                    this.notDelPanelId.push(...area.textPanelId.split(','));
                  }
                  if (area.compName === 'BtnsArea') {
                    ...
                    if (area.children.length) {
                      area.children.forEach((comp) => {
                        if (comp.textPanelId) {
                          this.notDelPanelId.push(
                            ...comp.textPanelId.split(','),
                          );
                        }
                          ...
                          smallArea.children.forEach((comp) => {
                            ...
                            if (comp.children) {
                              ...
                              comp.children.forEach((btn) => {
                                if (btn.textPanelId) {
                                  this.notDelPanelId.push(
                                    ...btn.textPanelId.split(','),
                                  );
                                }
                                btn.tabId = tab.compId;
                                obj[btn.compId] = JSON.parse(
                                  JSON.stringify(btn),
                                );
  ...
  mounted() {
    ...
    this.$store.dispatch(
      'getCurrentDict',
      'TRIGGER_EVENT,PANEL_TYPE,REQUISITE_TYPE',
    );
    // 更新字典项
  ...
  methods: {
    // 原型模式切换
    enabledPrototypeChange() {
      this.$set(this.configData[0], 'enabledPrototype', this.enabledPrototype);
    },
    // 预览页面1
    async previewPage() {
      ...
      try {
        if (this.isSelect) {
          await this.saveSelConfig();
        } else {
          await this.saveLayout(1);
        }
        this.showPreviewApp = true;
      } catch (error) {
        console.log(error);
      }
    },
    ...
    async initConfig() {
      ...
      if (this.isPanel && !this.isSelect) {
        let data = null;
        if (this.isUser) {
          data = await getDesignPage({
            menuId: this.sysMenuDesignId,
          });
          this.userConfigId = data.id;
        } else {
          data = await getSysDesignMenu({
            sysMenuDesignId: this.sysMenuDesignId,
          });
        }
        if (data && (data.designOverallLayout || data.designPageLayout)) {
          this.configData = data.designOverallLayout || data.designPageLayout;
          this.decryptSqlConfigData();
          ...
          this.$nextTick(() => {
            ...
            if (this.$route.query.isApp === '1') {
              ...
              [, this.activeObj] =
                this.configData[0].children[0].children[0].children;
    ...
    clearDirty() {
      ...
      if (this.configData[0].dictArr.length) {
        this.$store.dispatch(
          'getCurrentDict',
          this.configData[0].dictArr.join(','),
        );
      }
                    ...
                    if (area.compName === 'MultiTree') {
                      area.multiDataSource.forEach((sqlObj) => {
                        if (sqlObj.filterTermSql) {
                          sqlObj.filterTermSql = Encrypt(sqlObj.filterTermSql);
                        }
                      });
    ...
    decryptSqlConfigData() {
      try {
                      ...
                      if (area.compName === 'MultiTree') {
                        area.multiDataSource.forEach((sqlObj) => {
                          if (
                            ...
                            this.isJiami(sqlObj.filterTermSql)
                          ) {
                            sqlObj.filterTermSql = Decrypt(
                              sqlObj.filterTermSql,
                            );
                          }
                        });
                        ...
                        if (
                          area.deviceFilter.filterTermSql &&
                          this.isJiami(area.deviceFilter.filterTermSql)
                        ) {
                          area.deviceFilter.filterTermSql = Decrypt(
                            area.deviceFilter.filterTermSql,
                          );
                        }
                        if (
                          area.positionFilter.filterTermSql &&
                          this.isJiami(area.positionFilter.filterTermSql)
                        ) {
                          area.positionFilter.filterTermSql = Decrypt(
                            area.positionFilter.filterTermSql,
                          );
                        }
                      } else if (
                        area.filterTermSql &&
                        this.isJiami(area.filterTermSql)
                      ) {
                        area.filterTermSql = Decrypt(area.filterTermSql);
                      }
                      // 表格还需要解密汇总
                      if (
                        area.compName === 'TableMain' &&
                        ...
                        area.sumArr.forEach((item) => {
                          if (
                            item.filterTermSql &&
                            this.isJiami(item.filterTermSql)
                          ) {
                            item.filterTermSql = Decrypt(item.filterTermSql);
                          }
                        });
                      }
                    }
        ...
        if (this.configData[0].menuObj) {
          Object.keys(this.configData[0].menuObj).forEach((menuKey) => {
            const menu = this.configData[0].menuObj[menuKey];
            menu.forEach((m) => {
              m.menuFilter.forEach((mf) => {
                if (mf.filterTermSql && this.isJiami(mf.filterTermSql)) {
                  mf.filterTermSql = Decrypt(mf.filterTermSql);
                }
              });
            });
          });
        ...
        if (this.configData[0].paneObj) {
          Object.keys(this.configData[0].paneObj).forEach((paneKey) => {
            const pane = this.configData[0].paneObj[paneKey];
            pane.panelFilter.forEach((mf) => {
              if (mf.filterTermSql && this.isJiami(mf.filterTermSql)) {
                mf.filterTermSql = Decrypt(mf.filterTermSql);
              }
    ...
    packJson(configData) {
      ...
      const res = str.replace(/"([^,"]+?)":/g, (r) => {
        return r.replace(/[^":\\]+/g, (v) => {
          const index = objArr.key.indexOf(v);
          let packKey = '';
          if (index === -1) {
            objArr.key.push(v);
            let len = 1;
            let startIndex = 0;
            const allLength = v.length;
            while (!packKey || objArr.packKey.includes(packKey)) {
              if (len === allLength) {
                startIndex += 1;
              }
              packKey = `${v.slice(startIndex, len)}${allLength}`;
              len += 1;
            }
            objArr.packKey.push(packKey);
          } else {
            packKey = objArr.packKey[index];
          }
          return packKey;
        });
      });
      // console.log(res.length, JSON.stringify(objArr).length, str.length);
      const resObj = JSON.parse(res);
      resObj[0].objArr = objArr;
      this.unpackJson(resObj);
      return resObj;
    },
    unpackJson(configData) {
      const { objArr } = configData[0];
      delete configData[0].objArr;
      const str = JSON.stringify(configData);
      // eslint-disable-next-line arrow-body-style
      const res = str.replace(/"([^,"]+?)":/g, (r) => {
        return r.replace(/[^":\\]+/g, (v) => {
          const index = objArr.packKey.indexOf(v);
          const key = objArr.key[index];
          return key;
        });
      });
    ...
    async saveLayout(flag) {
      try {
        ...
        const configData = this.encryptSqlConfigData();
        // this.packJson(configData);
        if (this.isUser) {
          const a = {
            id: this.userConfigId,
            designPageLayout: configData,
            jobType: configData[0].userConfig.leaveJob,
          };
          const params = {
            jsonString: JSON.stringify(a),
          };
          await modifyDesignPage(params);
          this.$message({
            type: 'success',
            message: '保存成功',
          });
    ...
    async saveSelConfig() {
      try {
        ...
        if (
          this.configData[0].selShowType !== 4 &&
          this.configData[0].selShowType !== 2
        ) {
          if (this.$route.query.isApp === '1') {
            area.compType = 1002;
      ...
      for (let i = 0; i < length; i += 1) {
        ...
        switch (tempObj.compType) {
          case 'AREA':
            ...
            if (tempObj.compName === 'GanttMain') {
              if (!tempObj.taskColumn.startTime.id) {
                checkObj.flag = false;
                checkObj.msg = `${tempObj.name} 未配置开始时间字段`;
                break;
              }
              if (!tempObj.taskColumn.endTime.id) {
                checkObj.flag = false;
                checkObj.msg = `${tempObj.name} 未配置结束时间字段`;
                break;
              }
              // 周期性字段全部都有或者全部都没
              if (!tempObj.isTreeGantt) {
                if (
                  !(
                    (tempObj.taskColumn.interval.id &&
                      tempObj.taskColumn.intervalUnit.id &&
                      tempObj.taskColumn.cycle.id &&
                      tempObj.taskColumn.associatedColumn.id) ||
                    (!tempObj.taskColumn.interval.id &&
                      !tempObj.taskColumn.intervalUnit.id &&
                      tempObj.taskColumn.cycle.id &&
                      tempObj.taskColumn.associatedColumn.id)
                  ...
                  checkObj.flag = false;
                  checkObj.msg = `${tempObj.name} 3个周期性字段字段必须全有或者全为空`;
                  break;
                }
                if (tempObj.stateFilter) {
                  if (!tempObj.taskDict.id) {
                    checkObj.flag = false;
                    checkObj.msg = `${tempObj.name} 未配置状态字典`;
                    break;
                  }
                  if (!tempObj.taskColumn.state.id) {
                    checkObj.flag = false;
                    checkObj.msg = `${tempObj.name} 未配置状态字段`;
                    break;
                  }
                  if (!tempObj.taskColumn.state.lamba) {
                    checkObj.flag = false;
                    checkObj.msg = `${tempObj.name} 未配置关系符`;
                    break;
                  }
                  if (!tempObj.taskColumn.state.value) {
                    checkObj.flag = false;
                    checkObj.msg = `${tempObj.name} 未配置值`;
                    break;
                  }
                }
              } else if (
                !(tempObj.taskColumn.progress.id && tempObj.taskColumn.links.id)
              ) {
                checkObj.flag = false;
                checkObj.msg = `${tempObj.name} 进度字段跟链接线字段未配置`;
                break;
              }
          ...
          case 28:
            ...
            // } else
            if (
              tempObj.dataSource.tableName &&
              !tempObj.dataSource.columnName
            ) {
              checkObj.flag = false;
            ...
            if (tempObj.dataSourceType !== 2) {
              ...
              if (tempObj.enableMultiColumn) {
                if (tempObj.enableDict) {
                  console.log(tempObj.dataSource.dictObj);
                  if (!tempObj.dataSource.dictObj) {
                    checkObj.flag = false;
                    checkObj.msg = `${tempObj.name} 未选择字典`;
                    break;
          ...
          case 27:
            ...
            if (
              tempObj.dataSource.stepsArr &&
              !tempObj.dataSource.stepsArr.length
            ) {
              checkObj.flag = false;
          ...
          case 34:
            ...
            if (tempObj.activeValue === null) {
              checkObj.flag = false;
              checkObj.msg = `${tempObj.name} 未选择开启值`;
              break;
            }
            if (tempObj.inactiveValue === null) {
              checkObj.flag = false;
              checkObj.msg = `${tempObj.name} 未选择关闭值`;
              break;
            }
            break;
          default:
    ...
    resolveFilterTerm() {
      ...
      keys.forEach((key) => {
        ...
        if (this.getAllcheck[key].compType === 'AREA') {
          ...
          const arr1 = comp.filterTermSql
            ? comp.filterTermSql.match(reg) || []
            : [];
          const arr2 = comp.filterTermStr
            ? comp.filterTermStr.match(reg) || []
            : [];
  ...
  watch: {
    configData: {
      handler(v) {
        ...
        if (v[0].enabledPrototype) {
          this.enabledPrototype = true;
        } else {
          this.enabledPrototype = false;
        }
      },
      deep: true,
    },
