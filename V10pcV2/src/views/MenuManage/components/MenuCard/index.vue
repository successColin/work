  ...
  >
    <div class="menuCard__normal" v-if="curState === 1">
      <div class="menuCard__top">
        <icon-select
          v-if="item.icon"
          :key="item.icon.icon ? item.icon.icon : item.icon.imageUrl"
          :shouldScale="true"
          class="m-l-14 m-r-8"
          :notEdit="true"
          :item="item.icon"
        ...
        <div class="menuCard__normalText">
          ...
          >
            {{ item.menuName }}
            <span
              v-if="item.menuType !== 1 && !item.enabled"
              class="menuCard__normalText--canSee"
            >
...
export default {
  ...
  methods: {
    ...
    // 编辑菜单
    editMenu() {
      if (this.item.menuType === 1) {
        this.menuName = this.item.menuName;
        this.item.state = 3;
      } else {
        this.$emit('editMenu', this.item);
      }
    },
    moveMenu() {
