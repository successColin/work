...
export function getBlob(
  { url, token, method = 'get', contentType, dataType, data = '' },
  cb,
) {
  ...
  // 有菜单id加入头里面
  const menuId = window.vue.$route.params.id || window.vue.$route.query.menuId;
  if (menuId) {
...
export const passwordRule = [
  {
    value: 1,
    name: '大写字母、小写字母、数字、特殊字符(~!@#%_*)中任意一种且长度8位以上',
  },
  {
    value: 2,
    name: '大写字母、小写字母、数字、特殊字符(~!@#%_*)中任意两种且长度8位以上',
  },
  {
    value: 3,
    name: '大写字母、小写字母、数字、特殊字符(~!@#%_*)中任意三种且长度8位以上',
  },
  {
    value: 4,
    name: '大写字母、小写字母、数字、特殊字符(~!@#%_*)中四种且长度8位以上',
  },
];
