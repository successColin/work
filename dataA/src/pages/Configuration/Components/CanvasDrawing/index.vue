...
import {doAddHotDesign, editHotInfo, getHotInfo} from '@/services/design';
...
export default {
  ...
  methods: {
    ...
    addPoint(e) { // 添加自定义点
      ...
      if (this.activeShape) {
        let pos = this.canvasObject.getPointer(e.e);
    ...
    async addBlock(mouseForm, mouseTo) { // 添加区块
      ...
      try {
        const {id: elementId} = this.activeComponent;
        const {parentHost} = this.crumbsArr[this.crumbsArr.length - 1] || {};
        const res = await doAddHotDesign({name, elementId, parentHost: parentHost || 0, messageType: 1});
        newBlock.id = res;
    ...
    setCanvasZoom(zoom) {
      let center = this.canvasObject.getCenter();
      let corsurPoint = new fabric.Point(center.left * this.scale, center.top * this.scale);
