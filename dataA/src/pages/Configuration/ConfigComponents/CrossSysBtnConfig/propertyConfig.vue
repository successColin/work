...
<template>
  <div class="bgContentWrap">
    ...
    <el-collapse v-model="activeName" accordion>
      ...
      <el-collapse-item title="交互设置" name="3">
        <div>
          <div class="propsSetting">
            <p class="setTitle">接口地址</p>
            <c-input
                type="text"
                v-model="getComponentInfo.config.apiUrl"
                placeholder="请输入接口地址"
                @change="(value) => changeConfig(value, 'apiUrl')"
            >
            </c-input>
          </div>
          <div class="propsSetting">
            ...
            <c-select
                :options="requestOptionsCopy"
                :value="getComponentInfo.config.apiMethods"
                @change="(value) => changeConfigs(value, 'apiMethods')"
            >
            </c-select>
          </div>
          <div class="propsSetting">
            <p class="setTitle">参数管理</p>
          </div>
          <div class="propsSetting">
            <p class="setTitle">选择组件</p>
            <c-select
                :options="getComponetList"
                :value="getComponentInfo.config.apiComponent"
                @change="(value) => changeConfigs(value, 'apiComponent')"
            >
            </c-select>
          </div>
          <ParamsConfig
              ...
              @change="(v) => changeConfigs(v, 'apiParamsConfig')"
          ></ParamsConfig>
        </div>
      </el-collapse-item>
      <el-collapse-item title="其他" name="4">
        <div>
          <div class="propsSetting">
            <p class="setTitle">刷新方式</p>
            <c-select
                :options="reFreshOptions"
                :value="getComponentInfo.config.refreshType"
                @change="(value) => changeConfigs(value, 'refreshType')"
            >
            </c-select>
          </div>
        </div>
...
<script>
import {requestOptionsCopy} from '@/constants/global';
...
export default {
  ...
  computed: {
    getComponetList() {
      return this.getList.reduce((init, pre) => {
        const {componentName, name, componentId} = pre;
        if (componentName === 'InfoPresentation') {
          return init.concat([{...pre, value: componentId, label: name}])
        }
        return init;
      }, []);
    },
  ...
  mounted() {
    const {config: {apiComponent = '', apiParamsConfig = []}} = this.getComponentInfo;
    if (apiComponent) {
      this.getParamsOption(apiComponent);
    }
    this.paramsList = apiParamsConfig;
  },

  methods: {
    getParamsOption(v) {
      const obj = this.getList.find(({componentId}) => componentId === v);
      const {stylesObj: {labelConfig = []}} = obj;
      this.optionList = labelConfig.map((item) => {
        return {
          ...item,
    ...
    changeConfigs(v, k) {
      const obj = this.getComponentInfo.config || {};
      let newObj = {
        ...obj,
        [k]: v
      }
      if (k === 'apiComponent') {
        this.getParamsOption(v);
        newObj = {
          ...newObj,
          apiParamsConfig: []
        }
