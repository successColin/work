      ...
      <el-collapse-item name="1" title="图形属性">
        <div>
          ...
          <div class="propsSetting" style="margin-top: 10px;">
            <span class="setTitle">图形高度</span>
            <div>
              <c-input
                  type="number"
                  :numberValue="getComponentInfo.stylesObj.graphHeightScale"
                  :step="0.1"
                  :min="0.1"
                  :max="1"
                  @Input-Change="(value) => changeStyles(Number(value), 'graphHeightScale')"/>
            </div>
          </div>

          <div class="ellipsisWrap propsSetting">
            <span class="setTitle">自定义类型颜色</span>
            <div class="propsSetting">
            ...
            <CustomDisplay
                :customDisplay="getComponentInfo.stylesObj.colorArr"
                @change="changeStyles($event, 'colorArr')"
            />
            <div
                class="addWrap"
                style="margin: 0"
...
<!--          </div>-->
          ...
          <div class="propsSetting">
            <p class="setTitle">坐标轴分割间隔(天)</p>
            <c-input
                type="number"
                :numberValue="getComponentInfo.stylesObj.XInterval"
                :step="0.1"
                :min="0.1"
                @Input-Change="(value) => changeStyles(Number(value), 'XInterval')"/>
          </div>
...
import {getXAxisByKey, IsURL} from '@/utils/utils';
...
export default {
  ...
  computed: {
    ...
    getChartDataOptions() { // 获取图表的数据
      ...
      const legendData = getXAxisByKey(list, dimension === 'x' ? 's' : 'x'); // 确定图例
      if (!legendData.length) return [];
      return  legendData.map((item) => ({ value: item, label: item }));
    }
  },
  ...
  methods: {
    ...
    addCustomConfig() { // 新增自定义
      const { stylesObj: { colorArr = []} } = this.getComponentInfo;
      colorArr.push({
        c1: '',
        value: ''
      })
      this.changeStyles(colorArr, 'colorArr');
    },
