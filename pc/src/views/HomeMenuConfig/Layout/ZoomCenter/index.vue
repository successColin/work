...
export default {
  ...
  methods: {
    onEnd(e) {
      ...
      if (newIndex > oldIndex) { // 如果是前值往后移，目标位置的后面的图层等级需要修改
      ...
      if (newIndex < oldIndex) { // 如果是后面的数据往前移，一最新的数据为分割点，后面的数据后移
        cloneList.splice(oldIndex, 1);
        const lastArr = cloneList.slice(newIndex);
        const firstArr = cloneList.slice(0, newIndex);
        const { stylesObj: { zIndex } } = target;
    ...
    del() {
      ...
      this.$confirm(`此操作将删除${this.activeComponent.name}图层`, '是否继续?', '提示', {
        ...
        const index =
            this.list.findIndex((item) => item.componentId === this.activeComponent.componentId);
        const list = [...this.list];
        try {
    ...
    sendBackward() { // 下移一层
      ...
      const index = this.list
        .findIndex((item) => item.componentId === this.activeComponent.componentId);
      if (index === this.list.length - 1) { return; }
      const { stylesObj: { zIndex } } = this.list[index + 1];
      const obj = { ...this.activeComponent };
      const n = zIndex - 1;
