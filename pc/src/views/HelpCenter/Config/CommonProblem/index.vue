...
<template>
  ...
  >
    ...
    <!-- 表格 -->
    <apiot-table
      class="commonProblem__table"
      row-key="id"
      dropClass=".commonProblem"
      :tableData="tableData"
    ...
    <edit-drawer
      ...
      :placeholder="
        $t('placeholder.pleaseEnterAny', {
          any: this.$t('helpCenter.questionName'),
        })
      "
  ...
  computed: {
    ...
    drawerTitle() {
      return this.isAddDrawerState
        ? this.$t('common.add', { name: this.$t('helpCenter.qA') })
        : this.$t('common.edit', { name: this.$t('helpCenter.qA') });
    },
    // 多选数据ids的集合
    multiIdsString() {
      let str = '';
