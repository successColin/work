  ...
  <div class="drawer">
    ...
    >
      ...
      >
        ...
        <!-- 富文本 -->
        <el-form-item
          :label="$t('announce.announceContent')">
          <apiot-wangeditor
            ref="wangeditor"
...
export default {
  ...
  computed: {
    rules() {
      return {
        title: [{ required: true, message: this.$t('placeholder.pleaseEnterAny', { any: this.$t('announce.announcementTitle') }), trigger: 'change' }],
        startTime: [{ required: true, message: this.$t('placeholder.pleaseSelect', { any: this.$t('announce.enableTime') }), trigger: 'change' }],
        endTime: [{ required: true, message: this.$t('placeholder.pleaseSelect', { any: this.$t('announce.expirationTime') }), trigger: 'change' }],
      };
    },
  },
  components: {
  ...
  methods: {
    ...
    selectLeader(item) {
      ...
      const ids = this.curData.childIds ? this.curData.childIds : '';
      this.curData.childIds = `${ids}${item.id},`;
      this.isChecked = true;
      this.leaderKeywords = '';
      this.leaderList = [];
      if (!this.curData.childIds) {
