          ...
          >
            ...
            >
              <template slot="title">
                <div class="zoomTitle">
                  <div class="leftWrap">
                    <div><span class="iconfont icon-zongxiangtuodong"></span></div>
                    <div class="typeWrap">
                      <span
                        class="iconfont"
                        :style="`color: ${item.type === 'BIZ' ? '#5A80ED': '#EE5E5E'}`"
                        :class="{
                        [item.type==='FILE'?'icon-wenjianleixing':'icon-yewuleixing']:true}">
                      </span>
                    </div>
                    <div class="titleWrap">
                      层级{{i + 1}} ({{item.type === 'BIZ' ? '业务类型' : '文件类型'}})
                    </div>
                  </div>
                  <div class="rightWrap">
...
<!--                      </el-select>-->
                  ...
                  <el-table-column
                    ...
                    <template slot-scope="scope">
                      <span
                          @click="delColumns(scope.$index, item.columns)"
              ...
              <div v-if="item.type==='FILE'" class="fileWrap">
                <div>
                  <el-radio v-model="item.columns[0].name" label="createTime">资料上传时间</el-radio>
                  <el-select v-model="item.columns[0].format" placeholder="请选择">
                    <el-option
                        v-for="item in options"
                        :key="item.value"
                        :label="item.label"
                ...
                <div>
                  <el-radio v-model="item.columns[0].name" label="createUser">资料上传人</el-radio>
                </div>
              </div>
            </el-collapse-item>
          </transition-group>
...
export default {
  ...
  methods: {
    ...
    handleCommand(v) {
      ...
      const obj1 = {
        type: v,
        columns: [{ name: 'createTime', format: 'YYYY-MM-dd', dict: '', split: '' }]
      };
