      ...
      <div class="PanelConfig__btnArea m-t-10" v-if="curPaneObj.id">
        ...
        >
          <apiot-button
            ...
            :disabled="
              !Object.keys(canAddArea).length &&
              curPaneObj.panelFilter.length !== 0
            "
          >
...
export default {
  ...
  watch: {
    showScanConfig(v) {
      if (v) {
        const paneObj = JSON.parse(
          JSON.stringify(this.configData[0].paneObj[this.activeObj.compId])
        );
        if (
          this.activeObj.paneObj &&
          this.activeObj.paneObj.sysMenuDesignId &&
          paneObj.sysMenuDesignId === this.activeObj.paneObj.sysMenuDesignId
        ) {
          // 当前组件有配置，并且面板设计组等于跳转面板设计组
          this.curPaneObj = this.activeObj.paneObj;
          this.getPaneConfig();
        } else if (
          ...
          // 当前组件没配置，先去顶级取出面板id
          this.curPaneObj = paneObj;
          this.curPaneObj.panelData = [];
          this.curPaneObj.panelFilter = [];
          this.activeObj.paneObj = this.curPaneObj;
          this.getPaneConfig();
        }
      }
    }
