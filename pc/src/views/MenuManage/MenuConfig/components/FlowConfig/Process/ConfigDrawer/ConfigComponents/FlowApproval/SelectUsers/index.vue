        ...
        <div class="select__options">
          ...
          >
            ...
            >
              ...
              >
                <span>[人员]</span>创建人</el-menu-item
              >
              <el-menu-item
                :index="`${node.id}_3_${node.nodeTitle}`"
                v-if="[3, 5].includes(node.nodeType)"
              >
                <span>[人员]</span>执行人</el-menu-item
              >
            </el-submenu>
          </el-menu>
        </div>
      ...
      <ul class="flowDetailMembers" :key="updateKey">
        ...
        >
          <div class="flowDetailTagBox">
            ...
            <div class="flowDetailMemberArrow1"></div>
            <div class="flowDetailMemberArrow2"></div>
            <div
            ...
            <div class="flowDetailMemberTriangle"></div>
          </div>
            ...
            >
              <div style="height: 28px">
                <div class="Dropdown--input Dropdown--border">
                  <div class="lowDetailMemberTriangleTransparent"></div>
                  <div class="flowDetailMemberOrganizeTitle">
                    {{ renderCharge(confg.superiorType) }}
                  </div>
                  <div class="flowDetailMemberTriangleDark"></div>
                </div>
              </div>
            </div>
          </el-popover>
...
export default {
  ...
  methods: {
    deleteCharge(i) {
      ...
      NODE.splice(i, 1);
      this.$emit('select-flow-approval', { value: NODE, key: 'NODE' });
      this.updateKey += 1;
    },
    ...
    handleSure(users) {
      this.visible = false;
      this.$emit('select-flow-approval', { value: users, key: 'USER' });
    },
    handleSaveArr(visible, key, comKey) {
      // 组织，用户，职位 共用处理
      const { selectKeys } = this.$refs[comKey];
      this[visible] = false;
      this.$emit('select-flow-approval', { value: JSON.parse(JSON.stringify(selectKeys)), key });
    },
    ...
    deleteUser(userInfo) {
      ...
      USER.splice(index, 1);
      this.$emit('select-flow-approval', { value: USER, key: 'USER' });
    },
    // 删除选中组织
    deleteOrg(orgInfo) {
      ...
      ORG.splice(index, 1);
      this.$emit('select-flow-approval', { value: ORG, key: 'ORG' });
    },
    deleteRole(roleInfo) {
      // 删除选中的角色
      ...
      ROLE.splice(index, 1);
      this.$emit('select-flow-approval', { value: ROLE, key: 'ROLE' });
    },
    deletePos(posInfo) {
      // 删除选中的角色
      ...
      POST.splice(index, 1);
      this.$emit('select-flow-approval', { value: POST, key: 'POST' });
    },
    // 搜索
    async doSearch() {
