          ...
          <tr>
            ...
            <td colspan="3">
              <table class="childrenTable">
                ...
                <tbody>
                  <tr>
                    <td>{{ item.poolConf.initialSize }}</td>
                    <td>{{ item.poolConf.minIdle }}</td>
                    <td>{{ item.poolConf.maxActive }}</td>
                    <td>{{ item.poolConf.maxWait }}</td>
                    <td>{{ item.poolConf.timeBetweenEvictionRunsMillis }}</td>
                    <td>{{ item.poolConf.minEvictableIdleTimeMillis }}</td>
                  </tr>
                </tbody>
              </table>
            </td>
    ...
    dataSourceType() {
      return function(row) {
        if (this.DATASOURCE_TYPE.length) {
          ...
          const index = this.DATASOURCE_TYPE.findIndex((item) => item.value === temp);
    ...
    driverType() {
      return function(row) {
        if (this.DRIVER_TYPE.length) {
          ...
          const index = this.DRIVER_TYPE.findIndex((item) => item.value === temp);
