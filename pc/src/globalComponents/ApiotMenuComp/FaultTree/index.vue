...
import { listFaultTree, treeListPageList, selectList, getDesignMenu } from '@/api/menuConfig';
...
export default {
  ...
  computed: {
    ...
    getKeycodeId() {
      const index = this.getFeatureArr.children.findIndex(
        (comp) => comp.dataSource.columnName === 'keycode'
      );
      if (index !== -1) {
        return this.getFeatureArr.children[index].compId;
      }
  ...
  methods: {
    ...
    async getSidebarList(flag = true, dataType = 1, parentId = 1, needNext = true) {
      ...
      console.log(searchInfo, this.backKey);
      if (searchInfo && flag) {
        ...
        const res = await treeListPageList(params);
        ...
        data = await listFaultTree(params);
      }
      data.forEach((item) => {
        item.treeId = `${item.id}-${item.dataType}`;
        if (!item.childCount) {
    ...
    changeForm(data) {
      ...
      this.getFeatureArr.form[this.getKeycodeId] = data.keycode;
      this.$nextTick(() => {
    ...
    selectItem(item, flag = true, setFirst = true) {
      ...
      this.getFeatureArr.form[this.getKeycodeId] = item.keycode;
      if (flag) {
    ...
    async treeSave(isAdd) {
      ...
      if (isAdd) {
        ...
        } else if (form.dataType === 'G') {
          params = {
            selectContent: 'id,name,keycode,parent_id,root_path',
            selectFrom: 'sys_failure_relation',
            selectWhere: `sys_failure_relation.id=${form.id}`
          };
        } else if (form.dataType === 'A') {
          params = {
            selectContent: 'id,name,keycode,parent_id,root_path',
            selectFrom: 'sys_failure_phenomenon',
            selectWhere: `sys_failure_phenomenon.id=${form.nodeTableId}`
          };
        } else if (form.dataType === 'B') {
          params = {
            selectContent: 'id,name,keycode,parent_id,root_path',
            selectFrom: 'sys_failure_reason',
            selectWhere: `sys_failure_reason.id=${form.nodeTableId}`
          };
        } else if (form.dataType === 'C') {
          params = {
            selectContent: 'id,name,keycode,parent_id,root_path',
            selectFrom: 'sys_failure_measure',
            selectWhere: `sys_failure_measure.id=${form.nodeTableId}`
          };
        } else {
          params = {
