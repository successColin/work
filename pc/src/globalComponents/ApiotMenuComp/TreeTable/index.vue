...
export default {
  ...
  methods: {
    ...
    // 更新该区域
    reloadArea(areaArr, onlyFlag, compId, filterObj, isAdd, isTableBtn) {
      // 按钮全部刷新
      ...
      // 树表的更新
      if (areaArr === 'treeTableUpdate' && onlyFlag === this.onlyFlag()) {
        if (filterObj === 1) {
    ...
    resolveData(data, fatherUnique) {
      ...
      data.forEach((form) => {
        // unique 代表唯一值
        // notChange 代表没改变
        form.unique = this.configData.searchInfo ? createUnique() : form[this.getIdCompId];
        form.notChange = true;
        keys.forEach((key) => {
          // if (!form[key]) {
          //   form[key] = '';
