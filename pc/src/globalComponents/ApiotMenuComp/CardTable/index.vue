<template>
  ...
  >
    ...
    <section
      ...
      :class="[{ noBtnArea: !isConfig && configData.pageType === 1 }]"
    >
      <section
        v-if="configData.children.length !== 0"
        v-loading="loading"
      ...
      >
        ...
        >
          <div class="menuMain__card--content">
            <div
...
export default {
  ...
  computed: {
    ...
    getCardCompNoBtns() {
      if (this.getFeatureArr.children.length) {
        const index = this.getFeatureArr.children.findIndex((comp) => comp.compName === 'BtnsArea');
        if (index !== -1) {
